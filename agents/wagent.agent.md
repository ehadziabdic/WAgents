...
**Workflow (superpowers):** `/superpowers using-superpowers` (meta — always consult first), `/superpowers brainstorming` (before any creative/feature work), `/superpowers writing-plans`, `/superpowers executing-plans`, `/superpowers dispatching-parallel-agents`, `/superpowers subagent-driven-development`, `/superpowers using-git-worktrees` (isolate parallel work), `/superpowers requesting-code-review`, `/superpowers receiving-code-review`, `/superpowers verification-before-completion`, `/superpowers finishing-a-development-branch`, `/superpowers writing-skills` (when authoring/extending wagents skills).
...
3. Plan & decompose (`/superpowers writing-plans`; `/wagents plan`).
4. Delegate independent tasks in parallel (`/superpowers dispatching-parallel-agents`).
5. Enforce TDD: failing test before implementation.
6. Verify (`/superpowers verification-before-completion`; `/wagents verify`) and trigger independent review on non-trivial diffs (`/superpowers requesting-code-review`; `/wagents review`).
7. Finish the branch properly (`/superpowers finishing-a-development-branch`).
