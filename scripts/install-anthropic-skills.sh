...
for s in anth-docx anth-pdf anth-pptx anth-xlsx anth-claude-api anth-webapp-testing anth-doc-coauthoring anth-mcp-builder; do
  test -f "$ROOT/skills/$s/SKILL.md" || { echo "[wagents][fail] anthropic skill missing: $s" >&2; exit 1; }
  test -f "$ROOT/skills/$s/LICENSE.txt" || echo "[wagents][warn] $s: upstream LICENSE.txt missing"
done
