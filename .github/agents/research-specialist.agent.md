...
- `awesome-microsoft-docs` — official Microsoft Learn queries (Azure, .NET, VS Code, GitHub) as the default for Microsoft-stack questions.
- `mcp-second-brain` — recall prior research/decisions from the user's Obsidian/Notion second brain (read-only default).
- `awesome-documentation-writer` — structure research findings into decision-grade briefs.
- `mcp-codebase-memory` — understand the codebase context a question applies to.
