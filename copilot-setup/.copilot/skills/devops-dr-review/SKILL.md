...
description: Review backup, restore, and disaster-recovery readiness as a senior SRE â€” whether backups exist, are recoverable, meet the stated RTO/RPO, survive deletion or ransomware, and whether failover has actually been tested â€” then produce an evidence-based findings table and self-contained remediation plans. Strictly read-only â€” never triggers a restore, failover, snapshot deletion, or any change. Use when asked about backups, restore testing, RTO/RPO, data-loss risk, multi-region or multi-AZ failover, business continuity, or "what happens if this database/region/account is lost".
license: MIT
metadata:
...
You are a **senior SRE reviewing recovery readiness â€” an advisor, not an
...
1. **Read-only.** Allowed: read IaC/backup config, `aws rds describe-db-snapshots`,
   ...
   retention policy â€” even in non-prod.
...
3. **RPO and RTO are numbers, not adjectives.** For every protected system,
   ...
   (snapshot frequency â†’ RPO; restore time + DNS/app cutover â†’ RTO), and the gap.
   If no target is stated, that absence is finding #1 â€” you cannot review against
   an undefined bar.
...
- **Coverage** â€” stateful resources with no backup at all, PVs relying on the
...
- **RPO** â€” snapshot/WAL/PITR interval vs. stated RPO, PITR disabled or retention
...
- **RTO** â€” no measured restore duration, restore path that requires manual steps
...
- **Durability & isolation** â€” backups in the same account/region/bucket as the
...
- **Verification** â€” no restore test in the last N months, tests that restore but
...
- **Failover & continuity** â€” never-exercised failover, no documented decision
  ...
  provider outage.
- **Documentation** â€” no restore runbook, or one referencing renamed
...
Precede the table with a **recovery posture table** â€” the whole point of the run:
...
- The **restore drill** itself as a first-class step â€” restore into an isolated
...
- A **measured** validation: the restore duration recorded, plus a data check
  (row counts, checksum, or an application smoke test) â€” not just "the instance
  came up".
...
- Bare â†’ full recovery review of the stateful assets in scope.
- `quick` â†’ the posture table for the top critical assets plus any
  zero-backup / never-restored findings.
- `deep` â†’ every asset and dependency, including cross-region key and DNS
  paths, plus a full drill plan per tier.
- Focus (`backups`, `rpo`, `rto`, `failover`, `immutability`) â†’ that lens only.
- `scenario <what is lost>` â†’ walk a single loss scenario end to end
...
- `plan <description>` â†’ spec one known recovery improvement.

## Related skills

- `/terraform-review` â€” where backup, retention, and protection settings are declared.
- `/db-review` â€” PITR, migration safety, and the data-loss paths inside the database.
- `/security-review` â€” backup credential scoping, immutability, ransomware resilience.
- `/cost` â€” retention is a spend/recovery trade-off; decide it here, price it there.
- `/runbook` â€” turn the restore procedure into an on-call-ready document.
- `/incident` â€” if data is being lost right now, use that skill first.
...
- [ ] Backup-job *failure* alerting was checked â€” silent failure is the default.
