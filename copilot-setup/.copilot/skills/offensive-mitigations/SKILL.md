...
description: Use when assessing software or kernel hardening and determining which ASLR, DEP/NX, RELRO, canary, CFI, sandbox, or seccomp protections apply.
---
...
- **Intelâ€¯LAM:** Linear Address Masking support exists on recent kernels/CPUs but may be disabled by default. Verify with kernel config, boot params, and CPU flags on your target.
...
- Check with `Get-CimInstance -ClassName Win32_DeviceGuard` and `System Information â†’ Device Guard properties` for KDP/HVCI/VBS.
...
- Enable/verify: Windows Security â†’ Device Security â†’ Core isolation details.
...
- Android: enable perâ€‘app via Developer Options or `adb shell setprop persist.device_config.runtime_native_boot.mte_mode sync` (deviceâ€‘specific).
...
- Linux:
  - `mseal()` permanently seals selected VMAs so permissions/mappings can no longer changeâ€”even by the owner (verify kernel version and libc support on your target).
  - Adopted by projects such as Chrome/glibc/BPF toolâ€‘chains to seal JIT pages, locking down GOT/PLT and eBPF JIT regions (versionâ€‘specific; verify).

#### Bypass Techniques

- **Timeâ€‘ofâ€‘use Window:** Exploits must succeed before sealing.
- **Dataâ€‘only Abuse:** Still possible if the mapping remains writable.
...
- Windows:
  - Microsoft Pluton is increasingly deployed with newer platforms, replacing or augmenting discrete TPM 2.0 and hardwareâ€‘binding BitLocker keys, Secure Boot, and HVCI policies. Check OEM/SKU documentation for Copilot+ requirements.

#### Practitioner

- Check Pluton state in Device Manager â†’ Security devices, or `tpm.msc` shows Plutonâ€‘backed TPM if present.
...
- Linux/Intel:
  - Extends PKU to supervisor pages; the kernel flips page permissions via `wrmsr PKS_MSC*` without TLB flushes (Sapphireâ€‘Rapids+).
...
- Linux:
  - Recent kernels add support for stronger TPM transports over SPI/IÂ²C on some platforms. Feature availability and defaults vary; verify in `dmesg` and driver configs for your device.
...
- Firstâ€‘class Rust support landed in Linux 6.1 (December 2022) and was declared productionâ€‘ready with Linux 6.6 (October 2023).
- Inâ€‘tree Rust drivers (e.g., NVMe, DRM simpleâ€‘display, Wiâ€‘Fi) have so far exhibited zero memoryâ€‘safety bugs under continuous fuzzing, demonstrating the practical security benefit of memoryâ€‘safe languages.
...
- Starting in Windows 11 23H2, the Windows Driver Framework (WDF) officially supports both modern C++20 and a Rust projection (`windowsâ€‘driversâ€‘rs`) that wrap KMDF/WDF APIs with lifetimeâ€‘safe abstractions.
- Hardware vendors can now obtain WHQL signatures for C++20 or Rust kernels drivers, eliminating common lifetime and IRQLâ€‘misuse bugs without sacrificing performance.

### CHERI / Morello (Experimental Capability Hardware)

- Arm's Morello evaluation platform (2022â€‘2025) runs a CHERIâ€‘enabled Linux kernel that enforces pointer capabilities in user and kernel space, providing hardwareâ€‘enforced spatial and temporal memory safety.
- Although experimental, CHERI demonstrates a plausible postâ€‘2025 path toward fundamentally safer C/C++ code with architectural support.

### memfd_secret (userland secret memory)

- Linux:
  - `memfd_secret` (Linux 5.14+) provides userâ€‘mode pages hidden from other processes and the kernel direct mappings
...
### AMD Secure Encrypted Virtualization â€“ Secure Nested Paging (SEVâ€‘SNP)
...
- Cloud: verify TDX/SEVâ€‘SNP instance type (`Azure DCasv5/ECasv5`, `GCP C3`, `AWS C7g` variants); attest via platformâ€‘specific tools (e.g., `az confcom attestation`).
...
- pivot with `VirtualProtect` / `NtProtectVirtualMemory` (or preâ€‘ACG RWX section) from a ROP/JOP chain.
- Modern Windows 10/11 enforce **CET (Shadow Stack)** and **XFG (Crossâ€‘Function Guard)**, which break classic ROP; successful chains must first disable CET (for example with `SetProcessMitigationPolicy`) or switch to JOP/SCS gadgets.
...
- [Ropper 2.0](https://github.com/sashs/Ropper) **or Rizinâ€‘ropper** â€” both support CET/XFGâ€‘aware gadget filtering.
...
- Linux
  ...
  - **BHI Hardening (Linux 6.9/6.10):** FineIBT now incorporates indirectâ€‘branch serialization to mitigate Branch History Injection.
...
- Check process mitigation: `Get-ProcessMitigation -Name process.exe` (PowerShell) â†’ CFG/strictCFG/XFG states.
...
### Hardwareâ€‘Enforced Stack Protection
...
### Intel Controlâ€‘flow Enforcement Technology (CET)

- Hardware feature providing protections against control-flow hijacking.
- Includes Shadow Stack and Indirect Branch Tracking.
- Enabled by default starting with Windows 11 build 26100 (Win11 24H2+). Any stack pivot or `ret` without a valid shadowâ€‘stack token raises `STATUS_STACK_BUFFER_OVERRUN` (0xC0000409).
- **Detect CET:** Read the `IMAGE_DLLCHARACTERISTICS_GUARD_CF` flag in the PE header or `PEBâ†’LdrDataTableEntry.GuardFlags`.
...
- **Bypass tips for CET in shellcode:**
  ...
  2. Use ROPâ€‘less staging (queued APC, `NtContinue`, or `NtTestAlert`) so the kernel performs the first return.
...
- Linux:
  ...
  - Ensures only signed and verified code can execute in kernel mode.
  - Blocks RWX pages and unsigned kernel callbacks when **Memory Integrity** is ON (default on 2024â€‘hardware).
  - Common allocation pattern for executable memory: `PAGE_READWRITE` â†’ write payload â†’ `NtProtectVirtualMemory` â†’ `PAGE_EXECUTE_READ`.
  - WoW64 heavenâ€‘gate patches to `WOW64CFG` are rejected by HVCI; favour direct 64â€‘bit syscalls (e.g., via `wow64log`).

#### Bypass Techniques

- **Dataâ€‘only Attacks** (no returnâ€‘address writes).
- **Hypervisor/Kernel Bugs** that corrupt GCS state.

### Guarded Control Stack (GCS)

- Linux/ARM64:
  - Hardware userâ€‘space shadow stack on Armv9â€‘A CPUs, merged in Linux 6.13 and on by default in modern Android.
  - Complements KCET/CET on x86.

#### PANâ€‘GCS Dual Enforcement (Android 16, Armv9 Realms)

- From Android 16, GCS (shadow stack) plus Privileged Access Never (PAN) are **mandatory** for all Playâ€‘targetSdk 34+ apps running on Armv9 Realms hardware, providing dual hardware + software enforcement.
...
- Windows:
  - Debuted with Windows 11 23H2. Adds strict functionâ€‘prototype hashing to CFG, blocking many typeâ€‘confusion escapes.
...
#### KTRRâ€‘v2 & FastPAC (iOS 18, A18/A19)

- Hardwareâ€‘enforced cacheâ€‘colouring prevents pointerâ€‘authentication reâ€‘spray, complementing traditional KTRR for stricter kernel integrity.
...
- Windows:
  ...
  - **Safer allocation pattern to avoid AMSI heap scanning:** Reserve memory with `PAGE_NOACCESS`, decrypt/deobfuscate payload inâ€‘place, then change protection to `PAGE_EXECUTE_READ`.
  - The classic "patch the `AMSI` ASCII tag in `amsi.dll`" trick no longer works reliably; consider patching the COM VTable entry for `IAmsiStream::QueryInterface` or using a proxyâ€‘DLL hook instead.
...
- Audit perâ€‘process: `Get-ProcessMitigation` for DEP/ASLR/SEHOP/CFG/XFG.
...
- Seccomp userâ€‘notifier:
  - Enables brokerâ€‘style decisions in userspace; defend against confusedâ€‘deputy by strict validation and timeâ€‘bounded decisions
...
- Linux:
  ...
  - **Linux 6.14** extends lockdown to cover kexecâ€‘file pinning and loads the builtâ€‘in module blocklists earlier, further closing BYOVD avenues.

### Executableâ€‘policy Securebits

- **Linux 6.14:** Introduces `SECBIT_EXEC_RESTRICT_FILE` and `SECBIT_EXEC_DENY_INTERACTIVE` securebits together with the `AT_EXECVE_CHECK` flag, allowing interpreters to delegate final executionâ€‘permission checks to the kernel and tightening script/loader abuse paths.

### NTSYNC Driver Hardening

- **Linux 6.14:** The new `ntsync` driver offers an `io_uring`â€‘based fastâ€‘path that removes classic futex primitives from reachable attack surface, reducing userâ†’kernel synchronization abuse.
