...
description: Use when learning, teaching, or researching vulnerability classes and real-world CVE patterns such as overflows, UAF, integer bugs, format strings, type confusion, or races.
---
...
This document is Week 1 of a multiâ€‘week exploit development course, focusing on core vulnerability classes and realâ€‘world exploitation context.
...
- **Activities**:
  ...
  - _Concepts_:
    ...
    - The lifecycle of memory: allocation â†’ use â†’ deallocation
...
- **The Bug**: In the AMD display driverâ€™s EDID/VSDB parsing path, insufficient bounds checking allowed out-of-bounds reads when extracting identifiers, leading to slab-out-of-bounds access under KASAN.
...
- **The Attack**: An attacker with network access to the PAN-OS management web interface could send specially crafted requests that bypassed authentication logic. No credentials or user interaction were requiredâ€”the attacker could directly gain administrator access by exploiting the flaw in the authentication validation code.
...
**Case Study - CVE-2024-21338 (Windows AppLocker Driver Arbitrary Function Call â†’ Arbitrary Write)**:
...
- **The Attack**: An attacker could craft a malicious MP4/MOV media file with specially structured Theora extension data designed to trigger the signed integer underflow. The attack flow:
  ...
  5. `gst_buffer_new_allocate` â†’ `_sysmem_new_block` adds alignment/header to this huge size
...
- **Why It Matters**: This is a textbook example of signed-to-unsigned conversion vulnerabilities (CWE-195). In C/C++, implicit conversions between signed and unsigned integers follow complex rules that developers often misunderstand:
  ...
  The vulnerability demonstrates the full exploitation chain from integer bug to code execution: underflow â†’ type confusion â†’ allocation mismatch â†’ heap overflow â†’ structure corruption â†’ function pointer hijack. It's a perfect teaching example of why mixing signed and unsigned arithmetic in security-critical code is dangerous.
...
**What It Is**: Faulty userâ†”kernel marshalling and validation in system calls or device control paths (e.g., Windows `DeviceIoControl`, Linux `ioctl`).
...
- **The Bug**: A use-after-free in the Windows Win32k kernel subsystem allowed an authorized local attacker to elevate privileges. Classified under CWEâ€‘416, it was added to CISAâ€™s KEV in March 2025.
...
- **Examples from This Week**:
  - CVE-2024-27130 (QNAP): Stack overflow â†’ RCE
  - CVE-2024-2883 (Chrome ANGLE): UAF â†’ RCE via heap corruption
  - CVE-2023-4863 (libWebP): Heap overflow â†’ RCE
  - CVE-2024-7971 (V8): Type confusion â†’ RCE via arbitrary R/W
  - CVE-2023-35086 (ASUS Router): Format string â†’ RCE via stack overwrite

**Local Privilege Escalation (LPE)**:

- **Definition**: Attacker with limited access can gain higher privileges (user â†’ root/SYSTEM).
- **Impact**: High severity - allows persistence, defense evasion, lateral movement.
- **Examples**:
  - Windows Kernel TOCTOU (CVE-2024-26218): User â†’ SYSTEM via race condition exploitation
  - Driver IOCTL bugs: Standard user â†’ Administrator/SYSTEM
...
## Day 7: Capstone Project â€” Recent Vulnerability Triage and PoC Validation

- **Goal**: Apply the week's concepts by selecting several 2024â€“2025 vulnerabilities, correctly classifying and understanding them, locating and safely testing PoCs, and explaining what the vulnerability is, how it works, and how it can be chained.
- **Activities**:
  - **Select Targets (4â€“6 CVEs)**:
    - Choose recent CVEs from 2024â€“2025 spanning at least 3 categories: memory corruption, logic/auth, parser/media, kernel/drivers, networking/protocol, boot/supply chain.
  ...
  - **For Each CVE, Complete the Following**:
    ...
    4. **Explain (what/how/chain)**: In ~150â€“250 words, explain:
       - What the vulnerability is (class, component, versions, impact)
       - How it works (root cause and exploit path)
       - How it could be chained (e.g., info leak â†’ ASLR bypass â†’ RCE; sandbox escape â†’ LPE)
