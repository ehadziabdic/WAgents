...
description: Use when identifying software vulnerabilities through source review, black-box testing, taint analysis, dangerous-function hunting, data-flow tracing, or automated scanning.
---
...
- _Modern Linux interfaces (hotspots)_
  - **io_uring**: SQE size/offset confusions, submission/completion race windows, kernel copyâ€‘sizes derived from user buffers
  - **userfaultfd**: crossâ€‘thread writeâ€‘whatâ€‘where and TOCTOU primitives during fault handling
  - **seccomp userâ€‘notifier**: confusedâ€‘deputy patterns in broker processes; notifier timeâ€‘ofâ€‘check vs timeâ€‘ofâ€‘use gaps
- _Hyper-V & VTL Interfaces_ â€“ On many modern Windows 11 systems (especially 24H2 on supported hardware), Virtualizationâ€‘Based Security and VTL1 are enabled or easily enabled by policy. Treat the hypervisor surface (e.g., `hvix64.exe` and synthetic MSRs) as a common kernel target, and verify VBS/HVCI status on the host before assuming defaults.
...
- Practical triage example (CVEâ€‘2025â€‘8061):
  - IOCTL handlers that accept a fixedâ€‘size struct and pass a userâ€‘controlled `PHYSICAL_ADDRESS` directly to `MmMapIoSpace`
...
- **Userâ€‘entry vectors**: `bpf()` syscall, privileged pods in Kubernetes, Cilium datapath
- **Tooling**: `bpftool`, verifier logs, `bpftrace` scripts for quick triage
- **COâ€‘RE skeletons** (`bpftool gen skeleton`) simplify packaging portable tracing probes.
- **BPF LSM** hooks allow lowâ€‘overhead coverage feedback on securityâ€‘critical kernel paths; export events with `trace_pipe`.

### Container & Microâ€‘VM Surface

- Namespace/cgroup escapes, deviceâ€‘mapper abuse, races in snapshotting backends (e.g., overlayfs)
- Microâ€‘VM hypercalls in Firecracker, CloudHypervisor, Kata Containers
- For detailed container exploitation techniques, see [Container](/exploit/container.md)

### Cloudâ€‘Native & IAM Bugs

- Misconfigured IAM policies, privilegeâ€‘escalating API actions (AWS `sts:AssumeRole`, Azure Golden SAML)
- SSRF paths into metadata services (`169.254.169.254`, IMDSv2 bypass techniques)
- Race conditions in managed controlâ€‘plane components (Kubernetes API server, AWS Lambda workers)
...
- **QUIC / HTTP/3**: coalesced frames, reorder/timing corner cases; verify against RFCÂ 9000 (QUIC) and RFCÂ 9114 (HTTP/3)
- **HTTP/2**: stream state machine desync; flowâ€‘control integer edge cases (RFCÂ 7540)
...
- WASI sandbox escapes through hostâ€‘call interfaces
- Typedâ€‘Funcâ€‘Refs, GC, Tailâ€‘calls, Memory64 expand type/bounds confusion surface. See the WebAssembly proposals status page for current rollout and engine adoption.
- **Checklist**:
...
- **Turboshaft:** New IR replacing TurboFan internals; different optimization patterns create new bug classes
  ...
  - Maglev â†’ Turboshaft transition paths expose state inconsistencies
...
- **Renderer â†’ Browser requirements:** Need Mojo race or type confusion
...
### macOS / Appleâ€‘Silicon Kernel

- IOKit userâ€‘client input validation, IOMFB allocator cornerâ€‘cases
...
- Promptâ€‘injection, sandbox boundary escapes, hiddenâ€‘channel data exfil
- See [AI Security](/pentest/ai.md) for a deeper checklist

### Confidentialâ€‘Computing / TEE Surface

- **Intel TDX**: diff `tdx.ko` or `tdx_psci.c` between kernel LTS branches to spot new GPAâ†’HPA validation checks.
- **AMD SEVâ€‘SNP**: look for unchecked `VMGEXIT` leafs in PSP firmware; `sevtool --decode` helps locate IDA entry points.
...
- **HGXâ€¯HMC (verify CVE/advisories)**: research indicates malformed NVLINKâ€‘C2C packets can corrupt HMC register space; confirm against vendor advisories for the specific platform.
- **vGPU manager IOCTLs**: diff `nvidiaâ€‘vgpuâ€‘mgr` monthly; watch `VGPU_PLUGIN_IOCTL_GET_STATE` and similar calls for unchecked buffers.
- **LeftoverLocals infoâ€‘leak**: contiguous VRAM allocations can leak data from prior tenants in multiâ€‘tenant AI clusters.
...
#### Quick triage rubric (postâ€‘crash)

- Buffer overflow vs UAF: check access type, allocation lifetime, and redâ€‘zones (ASan/KASAN reports)
...
- Infoâ€‘leaks: uninitialized reads, OOB reads, pointer/string formatters

#### Coverageâ€‘first recon checklist

- Produce one baseline coverage run (e.g., `drcov`, IntelÂ® PT, or Lighthouse import)
...
#### Case Study: 7â€‘Zip Symlink Path Traversal

- Target: 7â€‘Zip 24.09 (vulnerable) â†’ 25.00 (fixed)
- File of interest: `CPP/7zip/UI/Common/ArchiveExtractCallback.cpp`
- Highâ€‘signal edits: absoluteâ€‘path detection and linkâ€‘path validation for WSL/Linux symlinks converted on Windows.

##### Minimal securityâ€‘relevant diff (simplified):
...
- Linux/WSL symlink data containing a Windowsâ€‘style path (e.g., `C:\...`) was treated as relative by the Linux absoluteâ€‘path check, setting `linkInfo.isRelative = true`.
- `SetFromLinkPath` prefixed the symlinkâ€™s zipâ€‘internal directory when building `relatPath`, letting `IsSafePath(relatPath)` pass despite an absolute Windows target.
- A subsequent â€œdangerous linkâ€ guard checked `_item.IsDir`; nonâ€‘directory symlinks skipped the validation.
...
- Verify absoluteâ€‘path detection across OS semantics (Linux vs Windows) and that relative/absolute status cannot be desynced by mixedâ€‘style paths.
- Ensure â€œdangerous linkâ€ checks run for both files and directories; avoid `_item.IsDir` shortâ€‘circuiting validation for file symlinks.
...
- Create zip structure:
  - `data/link` â†’ symlink to `C:\Users\<USER>\Desktop`
  - `data/link\calc.exe` â†’ payload file
...
#### WindowsÂ 11 Patch Diffing

This checklist mirrors the Apple IPSW workflow but uses Microsoft tooling and build numbers.

1. Identify the target update
   - Open **Settingsâ€¯â†’â€¯Windowsâ€¯Updateâ€¯â†’â€¯Update history** or consult the Windowsâ€¯Releaseâ€¯Health dashboard to note the **KB** and **OS build** numbers (e.g., _KB5037778Â â†’ buildâ€¯22631.3525_).
...
4. Load in the disassembler
   - Open _tcpip.sys_ from both **pre** and **post** folders in **IDAÂ 8+** or **GhidraÂ 11**; ensure PDB symbols resolve.
   - Save the IDA databases (e.g., `tcpip_3447.i64`, `tcpip_3525.i64`).

5. Run the diff
   - **BinDiffâ€¯7**: _Toolsâ€¯â†’â€¯BinDiffâ€¯â†’â€¯Diff Databaseâ€¦_ and select the two IDBs to generate a `.BinDiff` report.
   - **Ghidriff** (headless):
     ```bash
...
6. Triage the results
   - Sort by _Similarityâ€¯%_ ascending; investigate anything below **95â€¯%**.
   - Focus on functions with names like `Validate`, `Parse`, `Copy`, `Check`, or protocolâ€‘specific handlers (`IppReceiveEsp`, `Ipv6pFragmentReassemble`, etc.).
   - Determine whether changes add bounds checks, size validations, or privilege checks.

7. Validate in a lab VM
   - Snapshot two Windowsâ€¯11 VMs (build **3447** and **3525**).
   - Attach WinDbg (kernel mode) using `bcdedit /dbgsettings net hostip:<IP> port:<PORT>`.
   - Reproduce the issue against the **preâ€‘patch** VM; confirm no crash or breakpoint triggers in the **postâ€‘patch** VM.
...
Patchâ€‘diffing Linux kernels is often faster at the source level, but for binaryâ€‘only targets (vendor kernels, modules) functionâ€‘level diffing is still practical.
...
6. Sourceâ€‘level triage (when sources are available)
...
7. Symbolization and crash mapping (cheatâ€‘sheet)
...
> [!TIP]
> For modern distros built with Clang: KCFI and fineâ€‘grained CFI thunks create many small stub changes; filter by real function body deltas to focus on securityâ€‘relevant logic.
...
- Pattern to flag: `e = (struct entry *)((char *)e + next);` without a preceding block that revalidates `buf_len` and the entryâ€™s internal sub-lengths.
...
- Lessons distilled from a 2025 ksmbd remote chain writeup combining a fixed-buffer overflow in NTLM auth with an EA `next` validation issue â€” see Willâ€™s Root: Eternalâ€‘Tux: KSMBD 0â€‘Click RCE (`https://www.willsroot.io/2025/09/ksmbd-0-click.html`).
...
- **Semgrep Pro** â€“ Cloud-augmented SAST with custom rule sharing, LLM-assisted rule writing
- **CodeQL** â€“ GitHub's semantic code analysis, excellent for variant analysis
- **Weggli** â€“ Fast semantic search for C/C++ (better than grep for code patterns)
- **Joern** â€“ Code property graph analysis for vulnerability discovery
- **Ghidra 11.2+** â€“ Built-in ML-powered function signature recognition
- **Binary Ninja 4.0** â€“ Cloud collaboration, improved HLIL decompilation
- **Cutter** â€“ Rizin GUI with built-in decompiler integration

#### Deprecated Tools (Avoid)

- **Intel Pin** â†’ Use DynamoRIO or Frida (Pin is sustain-only mode)
- **WinAFL** â†’ Use AFL++ 4.x (integrated Windows support)
- **Old BinDiff** â†’ Use BinDiff 8 or Ghidriff
...
- [Intel Pin](https://www.intel.com/content/www/us/en/developer/articles/tool/pin-a-dynamic-binary-instrumentation-tool.html): is a dynamic binary instrumentation framework
  ...
  - Can be combined with Ghidra/IDA to provide more information
    > [!NOTE]
    > Intelâ€¯Pin is now in sustainâ€‘only mode â€” no new ISA extensions will be added. Prefer **DynamoRIO** or **Fridaâ€‘Stalker** for forwardâ€‘looking projects.
...
### eBPFâ€‘based Dynamic Tracing

- **bpftrace** / **BCC**: attach uprobes/kprobes to collect heap, syscall, or scheduler events with minimal overhead
- Ideal for preâ€‘fuzz triage and live coverage measurement on kernels or containerised workloads
...
- Cluster crashes with tokenâ€‘based Capstoneâ€‘hashing or `gdbâ€‘script dedup.py` before manual analysis.

### Fuzzing Tools

#### Modern Fuzzing Frameworks

- **AFL++ 4.21+** â€“ Unified cross-platform fuzzer with Windows support, CMPLOG, QEMU-mode
- **LibAFL 0.13+** â€“ Rust-native, highly customizable, supports in-process and fork-server modes
- **Honggfuzz** â€“ Persistent-mode Windows support, hardware-based feedback
- **Nyx** â€“ Full-VM snapshot fuzzing with KVM acceleration
- **ICICLE** â€“ Fast Windows kernel fuzzing framework
- **Syzkaller** â€“ Kernel fuzzer with LLM-guided seed selection (2025)

#### Instrumentation & Coverage

- **DynamoRIO** â€“ Used by AFL++, faster than Intel Pin (Note: Intel Pin is now sustain-only)
- **Frida Stalker** â€“ Cross-platform dynamic instrumentation
- **Intel PT** â€“ Hardware-accelerated coverage collection
- **Emerald** â€“ Generates `drcov` data for coverage visualization

#### Specialized Fuzzers

- **ChatAFL / LLM-driller** â€“ LLM-guided corpus expansion (+30% coverage on complex targets)
- **Reads-From Fuzzer (RFF)** â€“ Concurrency fuzzer for race/TOCTOU bugs
- **LibFuzzer** â€“ In-process, coverage-guided fuzzer for source-available targets
- **Radamsa** â†’ **Replaced by LibAFL mutators** (more coverage-aware)

#### Symbolic Execution Engines

- **Triton** â€“ Dynamic binary analysis framework
- **Angr** â€“ Binary analysis platform with symbolic execution
- **Manticore** â€“ Dynamic symbolic execution tool
- **S2E** â€“ Selective symbolic execution platform

### Continuous-Integration Fuzzing

- **ClusterFuzzLite** â€“ GitHub Actions/CI runner that feeds corpora to AFL++, libFuzzer or honggfuzz and files issues automatically.
...
- Systematically permutes thread scheduling to uncover dataâ€‘race, atomicity, and TOCTOU vulnerabilities that traditional coverageâ€‘guided fuzzers miss

#### LLMâ€‘Guided Fuzzing

- **ChatAFL** â€“ integrates an LLM to propose protocolâ€‘aware mutations; boosts state coverage on network daemons by ~40â€¯%.
- **SyzAgent / SyzLLM (2025â€‘02)** â€“ schedules kernel `syz` programs suggested by an LLM fineâ€‘tuned on the Syzkaller corpus.
- **NumSeed** â€“ leverages naturalâ€‘language descriptions of inputs to seed generation for binaryâ€‘only targets.
...
- **HyLLFuzz:** GPT/Llama-3 generates branch-targeted mutations achieving ~1.3Ã— edge coverage improvement
