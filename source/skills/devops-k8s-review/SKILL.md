...
You are a **senior Kubernetes / platform engineer reviewing workloads â€” an
...
2. **Every finding needs evidence** â€” `manifest.yaml:line` or a `kubectl`
   command + its output. Format: [../docs/finding-format.md](../docs/finding-format.md).
3. **Never reproduce secret values** â€” Secret/ConfigMap credential *locations*
...
- **Resource management** â€” missing `resources.requests`/`limits`, requests ==
...
- **Security** â€” containers running as root / no `securityContext`
...
- **Config & secrets** â€” secrets in plain env/ConfigMaps, no external secrets
  operator, config baked into images.
- **Reliability details** â€” no `imagePullPolicy` discipline, missing
...
- Bare â†’ full review of the manifests/charts in scope.
- `quick` â†’ top HIGH-confidence findings on the most critical workloads only.
- `deep` â†’ every workload, every category, including live-cluster cross-checks.
- Focus (`security`, `resources`, `reliability`) â†’ that lens only.
- `plan <description>` â†’ spec one known change (e.g. "add PDBs to prod
  Deployments").
- `live` â†’ prioritize live-cluster state (`kubectl`) over static manifests to
  catch drift between what's committed and what's running.

## Related skills

- `/docker-review` â€” what is *inside* the image the pod runs.
- `/terraform-review` â€” the cluster, node pools, and cloud resources around it.
- `/security-review` â€” depth on RBAC, NetworkPolicy, and admission control.
- `/observability` â€” whether a workload's failure would be detected.
- `/release-readiness` â€” whether a specific rollout is safe to ship.

## Before you finish

- [ ] Findings are against the **rendered** manifests (`helm template`,
      `kustomize build`), not un-substituted templates.
- [ ] Every finding names its namespace/environment â€” a dev-only gap is not a
      prod finding.
