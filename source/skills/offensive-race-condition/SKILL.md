...
description: Use when testing web applications for TOCTOU windows, concurrent-request races, rate-limit bypasses, double spends, or last-byte timing behavior.
---
...
- **Single-Packet Attack (HTTP/2)** and **Last-Byte-Sync (HTTP/1)** research (PortSwigger Black Hat 2023) enables â‰¤ 4 Âµs request skew; both are now directly supported in Burp Repeater and Turbo Intruder.
...
- **Specialized Tools**:
  ...
  - Raceocat: CLI scanner that replays raw-socket requests for Âµs-precision
