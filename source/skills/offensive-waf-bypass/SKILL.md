...
description: Use when a WAF blocks authorized web-testing payloads and encoding, syntax, headers, chunking, rotation, timing, or obfuscation behavior needs assessment.
---
...
- **Cloudflare** - Identified by `__cf_bm`, `cf_clearance`, `cf_chl_*` cookies and "/cdn-cgi/" routes (cookies rotate roughly every 30â€¯min)
- **Akamai**
...
- **Cloudflare AI WAF**
- **Radware**
- **Coraza** - Modern openâ€‘source WAF written in Go
...
6. **JA3/JA4/TLS Fingerprints** - Compare your clientâ€™s TLS fingerprint with common browser fingerprints; mismatches are often blocked.
...
- **Fastly Nextâ€‘Gen WAF**: `fastly-debug-*` headers (when enabled), service IDs in responses
...
- Check historical DNS records (e.g., with `securitytrails` API) â€“ 2024 research found ~40â€¯% of Fortuneâ€‘100 origins exposed via stale A records
...
- **Cloudflare Turnstile** â€“ lowâ€‘friction CAPTCHA replacement; see `cloudflare_turnstile_bypass` PoC (GUIâ€‘driven, YMMV)
...
- **Alternate encodings**:
  - URL encoding: `UNION` â†’ `%55%4E%49%4F%4E`
  - Hex encoding: `SELECT` â†’ `0x53454C454354`
  - Unicode encoding
...
- Leverage mixed encodings (halfâ€‘width Unicode, overlong UTFâ€‘8), embedded comments, and case folding differences.
...
- **Host Header Spoofing**:
  ...
  - **Protection**:
    ...
    - Enforce strict ALPN and certificate SAN checks; for HTTP/2 enforce `:authority` â‰ˆ SNI.
...
- **WAF Fuzzing Automation**:
  ...
  - Add feedbackâ€‘driven payload evolution (genetic algorithms) to evade MLâ€‘based detections.
...
- **Advanced Encoding Techniques**:
  ...
  - **MIME Smuggling**: multipart boundary tricks, mixed `Contentâ€‘Type` declarations through chained proxies.
...
- **Timing-Based Techniques**:
  ...
  - Use request coalescing and longâ€‘lived HTTP/2 streams to alter inspection timing.
...
- **Machine Learning-Based Evasion**:
  ...
  - Cloudflare **AI WAF Attack Score** can be tricked by embedding payloads in lesserâ€‘weighted features (e.g., multipart filenames)
  - Vary entropy and payload tokenization to mislead nâ€‘gram/embedding models.
...
- **HTTP/2 Rapid Reset flood** (CVEâ€‘2023â€‘44487): send a burst of `RST_STREAM` frames to overwhelm backâ€‘ends while frontâ€‘end WAFs proxy at HTTP/1.1.
- **HTTP/2 CONTINUATION Flooding** (2024 research): stress header compression/HPACK handling on middleboxes.

### 17. GraphQLâ€‘Specific WAF Bypasses

- **Introspection & alias spamming**: use deeply nested aliases to evade depth limits.
- **Batch operations**: bundle many queries in one request to sneak payloads through superficial queryâ€‘string filters.
- **Query name tampering**: mask malicious fields behind benign names.
- **Tools**: `graphqlâ€‘cop`, `inql`, `Escape pâ€‘cli` for automated fuzzing.
...
- Mixed: Some vendor POPs or partner edges may behave differently; validate perâ€‘region behavior.
...
<Ñ•cript>alert(1)</Ñ•cript>  # Cyrillic 's' characters
...
- **graphqlâ€‘cop** â€“ Fuzzer for GraphQL APIs with WAF bypass testing
- **GoReplay/Mitmproxy** â€“ record & replay traffic through different network paths to compare WAF decisions
...
- **nobleâ€‘tls / uTLS / tls-client** â€“ spoof browserâ€‘grade TLS stacks programmatically
