...
description: Use when planning or conducting structured OSINT work involving target scoping, source selection, collection, correlation, timeline reconstruction, or reporting.
---
...
- Resources
  ...
  - Use separate browser profiles or isolation tools (e.g., **Firefox Multiâ€‘Account Containers**) for any sockâ€‘puppet activity.
  ...
  - Audit every browser extension before installation; supplyâ€‘chain attacks on popular addâ€‘ons have targeted investigators since 2024.
  - Use dedicated browser profiles/containers per case and persona; avoid logging into personal accounts.
  - Prefer hardwareâ€‘backed passkeys for critical accounts; store recovery codes offline.
  - Maintain a minimal chainâ€‘ofâ€‘custody: timestamp actions, hash key artifacts, and record tool versions per case.
...
- Tools:
  ...
  - Chainalysis: **Horizon 2.0** crossâ€‘chain tracing suite (paid)
  - Elliptic: **Lens** visual link explorer (launched Dec 2024)
  - Most compliance suites now provide **realâ€‘time bridgeâ€‘risk scoring** dashboards (e.g., TRM, Chainalysis)
...
- Bridges/mixers/wrappers introduce mint/burn semantics; avoid assuming 1:1 flows without onâ€‘chain proofs.
- MEV/sandwich and aggregator paths can create false "direct" trails; validate with multiple datasets.
- Crossâ€‘label sanity: vendor labels can disagree; treat labels as hypotheses, not ground truth.
...
- Contextual Analysis
  - Use multiple reverse image search engines to find matches or similar images:
    ...
    - [Copyseeker](https://copyseeker.com/) AIâ€‘based reverseâ€‘image search engine
...
- Trial and Error
  ...
  - Use Snap Map public stories for areaâ€‘based context pivots.
...
- Tools
  ...
  - [Exposing.ai](https://exposing.ai/) facialâ€‘dataset search
...
- [ADS-B Exchange](https://www.adsbexchange.com/) â€“ unfiltered global flight data
...
- Platform-Specific Techniques
  - TikTok and Instagram
    ...
    - Sample cadence: 1â€“4 h for fastâ€‘moving topics; keep a fixed persona and capture logs
...
- Auditory Clues
  ...
  - Tools
    ...
    - [SoundCMD](https://soundcmd.com/) crowdâ€‘sourced soundâ€‘matching engine
  - Methodology
...
- Analyze frames using the same techniques as in image geolocation.
  - When possible, obtain the original upload (avoid reâ€‘encodes) to retain metadata and audio clarity.
  - Decode platform snowflakes (e.g., Discord, Twitter/X) to infer serverâ€‘side timestamps for events.
...
- Tools
  ...
  - [ShadeMap](https://shademap.app/) â€“ interactive 3â€‘D shadow simulator
  - Bellingcat **Shadowâ€‘Finder** microâ€‘tool
  ...
  - OSM mapâ€‘compare sites and EOX Cloudless layers to crossâ€‘check base imagery.
...
### Actorâ€‘Centric Workflow

- Scoping
  - Define the actor hypothesis (e.g., APT28, APT29, Turla, Sandworm; APT10, APT41, MustangÂ Panda, VoltÂ Typhoon).
  - Collect seed reports from CERTs and vendors; extract indicators and TTPs.
- Indicator harvesting
  - Parse IOCs (domains, IPs, hashes, JA3/JA4, userâ€‘agents) from advisories and reports; normalize and deâ€‘duplicate.
  - Validate IOCs with passive DNS, CT logs, sandbox submissions, and open telemetry where possible.
- Infrastructure mapping
  - Build pivots from CT logs (SANs, issuer, serials), shared hosting, nameâ€‘server reuse, registrar accounts, and HTML/page fingerprints.
  - Enrich with ASN/WHOIS history, RPKI/ROA status, geolocation, and hosting provider relationships.
- Artifact profiling
  - Extract PE/ELF metadata (PDB paths, compile timestamps, Rich headers, resources language, codeâ€‘signing certs).
  - Cluster with fuzzy hashes (SSDEEP/TLSH) and identify packers/loaders; search YARA and sandboxes for nearâ€‘matches.
...
- Falsification and reporting
  - Weigh each linkage (weak/medium/strong); document alternatives; avoid singleâ€‘source attribution.
  - Map TTPs to MITRE ATT&CK and cite sources with exact sections/pages.

### Attribution Discipline

- Separate capability from intent and sponsorship; avoid mirrorâ€‘imaging.
- Use a ruleâ€‘ofâ€‘three: require at least three independent weak signals, or one strong + one weak, before asserting linkage.
- Prefer durable pivots (registrar accounts, codeâ€‘signing cert reuse, build path idioms) over ephemeral ones (resolving IPs).
- Clearly mark uncertainty levels and confidence (e.g., low/medium/high) and distinguish correlation from control.

### Russiaâ€‘Specific Pivots

- Corporate/people
  - EGRUL/EGRIP extracts (official registry; captchaâ€‘gated) and Rusprofile/Kontur.Focus summaries for entities and directors.
...
- Infrastructure
  - RU WHOIS: `whois.tcinet.ru`; check registrar accounts, nserver patterns, and RUâ€‘center usage.
  - Telegram is widely used; analyze channels, admins, crossâ€‘posts, and bot ecosystems.
- Media/platforms
  - VKontakte, Odnoklassniki, Rutube, and regional news portals; search in Russian and transliterations.

### Chinaâ€‘Specific Pivots

- Corporate/people
  - National Enterprise Credit Info System (`gsxt.gov.cn`) for registered entities; crossâ€‘check with Tianyancha/Qichacha (paid/freemium).
...
- Static triage
  - Hash (SHAâ€‘256), strings, import tables, PDB path, Rich header, resources; check VT/Malpedia family hints (do not rely solely on AV labels).
- Dynamic/sandbox
...
- WeChat
  ...
  - Expect link rot and content takedownsâ€”archive early.

## Automation & Case Management

- Tools:
  - [Hunchly](https://www.hunch.ly/) (browser evidence capture)
  - [Kasm Workspaces](https://kasmweb.com/) OSINTâ€‘ready workspace images
  - [ArchiveBox](https://archivebox.io/) â€“ selfâ€‘hosted web archiver
