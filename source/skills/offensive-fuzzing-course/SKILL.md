...
description: Use when planning or running fuzzing campaigns, building corpora and harnesses, or triaging crashes from AFL++, libFuzzer, or coverage-guided fuzzing.
---
...
This document is Week 2 of a multiâ€‘week exploit development course, focusing on discovering vulnerabilities through fuzzing techniques and analyzing the crashes to determine exploitability.
...
Fuzzing can feel a bit frontâ€‘loaded: you may spend time wiring harnesses and running campaigns without immediately finding exciting new bugs, especially on hardened or wellâ€‘tested targets. Thatâ€™s normal, and it's one reason the next week on patch diffing often feels more directly "practical" â€” many companies already run large fuzzing setups and need people who can understand and exploit the bugs those systems uncover. Still, working through this week is important: it teaches you how fuzzers actually discover real vulnerabilities, so when you later triage crashes or study patches, you'll have a solid intuition for how those bugs were found and how to reproduce them.
...
#   - 32-bit signed 'size' underflows â†’ huge unsigned value
#   - _sysmem_new_block() overflows when adding alignment/header â†’ tiny (0x89-byte) allocation
...
- **Activities**:
  ...
  - _Exercises_:
    ...
    3. Extend the fuzz target to cover a small parser-style function, similar to the image/format parsers from Days 1â€“2.
...
This is exactly the same pattern as our AFL++ labs: **fuzzer + sanitizer â†’ crash â†’ root cause â†’ fix**, but now entirely inside a unit test binary.

### Lab 3: Fuzzing a small parser-style function

To connect FuzzTest to the real-world parsers from Days 1â€“2, fuzz a tiny length-prefixed parser that can easily go wrong if you mishandle integer arithmetic.
...
3. **Great fit for parsers and core logic**: Short, pure C++ functions (parsers, decoders, protocol handlers) are ideal FuzzTest targets, similar to the real-world parsers from Days 1â€“2.
4. **Properties > examples**: Expressing invariants like â€œnever crashâ€ or â€œlength field matches payloadâ€ lets the fuzzer explore inputs you would never hand-write.
5. **Same workflow as other fuzzers**: Regardless of tool (AFL++, Honggfuzz, FuzzTest), the basic loop is still _fuzz â†’ crash â†’ triage â†’ exploitability â†’ fix_.
...
# Result: Timeout on malformed EC point â†’ DoS vulnerability
```
...
2. **Automated ratings are heuristics**: CASRâ€™s `Type`/`Severity` fields are a **starting point only**; Case Study 2 showed a UAF rated `NOT_EXPLOITABLE` even though a function pointer hijack is clearly possible.
...
5. **Systematic PoC development**: The path is always _fuzz â†’ crash â†’ triage (ASan + CASR) â†’ root cause â†’ minimal reproducer â†’ exploit PoC (heap metadata or function pointer overwrite)_.
...
2. How could CASRâ€™s `NOT_EXPLOITABLE` rating for the UAF case mislead a less experienced analyst, and what manual checks (in GDB) prevent that mistake?
...
5. How do modern mitigations (ASLR, DEP, hardened allocators, CFI) interact with the exploitation strategies you used in Case Studies 1â€“3, and what extra steps would be needed in a real target?
...
- [Slice: SAST + LLM Interprocedural Context Extractor](https://noperator.dev/posts/slice/) - Using build-free CodeQL + Tree-Sitter + GPTâ€‘5 to triage ~1700 static UAF candidates in the Linux kernel down to a single real bug (CVE-2025-37778), a good complement to fuzzing-based crash triage.
...
5. **Continuous Integration**: Tools like OSS-Fuzz automate the "find â†’ fix â†’ verify" loop, preventing regressions in evolved code.
