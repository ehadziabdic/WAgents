---
name: offensive-idor
description: Use when testing APIs or web applications for predictable identifiers, object enumeration, broken access control, or horizontal and vertical privilege escalation.
---
...
2. **Establish Baseline Behavior**:
   ...
   - Evaluate caching headers (ETag/Last-Modified) that can leak existence sideâ€‘channels during enumeration

3. **Request Capture Setup**:
   - Configure a proxy (e.g., Burp Suite, OWASP ZAP)
   - For mobile applications, install the proxyâ€™s CA certificate on the device or emulator (e.g., with mitmproxy or Burp Mobile Assistant) so HTTPS traffic can be intercepted.
...
1. **Request Parameter Analysis**:
   ...
   - Common parameter names:
     ...
     ```
   - Watch for identifiers hidden in JWT claims (`sub`, `org_id`) or signed cookies; tamper if server fails to reâ€‘authorize.

2. **Parameter Manipulation Techniques**:
   - **Direct Modification**: Change numerical IDs (e.g., `id=1` â†’ `id=2`)
   - **Add Missing IDs**: Try adding relevant IDs (e.g., `user_id`, `account_id`) to requests that don't initially have them (e.g., `GET /api/messages` â†’ `GET /api/messages?user_id=<victim_uuid>`). Parameter names can often be inferred from other requests or discovered using tools like Arjun.
   ...
   - **Array/Object Manipulation**: Add or modify array elements in API requests
     ```json
     {"items": [{"id": 123, "owner": "victim"}]} â†’ {"items": [{"id": 456, "owner": "attacker"}]}
     ```
   ...
   - **Array-based Access**: Try wrapping IDs in arrays (e.g., `{"id":19}` â†’ `{"id":[19]}`).
   - **JSON Object Wrapping**: Try wrapping the ID in a nested JSON object (e.g., `{"id":111}` â†’ `{"id":{"id":111}}`).
   - **Numeric vs Non-Numeric IDs**: If the application uses non-numeric IDs (GUIDs, usernames), try substituting them with potential numeric equivalents (e.g., `account_id=UUID` â†’ `account_id=123`).
   - **Parameter Name Replacement**: Try alternative parameter names (e.g., album_id â†’ account_id). Fuzz JSON Patch (RFC 6902) and JSON Merge Patch (RFC 7386) bodies for crossâ€‘user modifications.
...
5. **Web Socket Discovery**:
   ...
   - Inspect mobile deep links and intent filters that include object IDs; try crossâ€‘app invocation.
...
- **Encoded IDs**: Decode base64/hex encodings and modify values
  ```
  /api/document/MjQ2 (base64 of "246") â†’ /api/document/MjQ3 (base64 of "247")
  ```
...
- **HTTP Method Switching**: Change GET to POST or PUT
  ```
  GET /api/users/123 â†’ POST /api/users/123
  ```
- **Content-Type Manipulation**: Modify content-type headers
  ```
  Content-Type: application/json â†’ Content-Type: application/xml
  ```
...
- **Outdated API Version Testing**: If multiple API versions exist (e.g., /v1/, /v2/), test the IDOR on older, potentially less secure versions.
  ```
  GET /v3/users/123 â†’ 403 Forbidden
  GET /v1/users/123 â†’ 200 OK
  ```
...
- **RESTful API IDORs**: Direct manipulation of resource identifiers in REST APIs
  ...
  ```
  GET /api/users/123/profile â†’ GET /api/users/456/profile
  ```
...
- **JSON/XML Parameter IDORs**: Modifying nested parameters
  ```json
  {"user": {"id": 123, "action": "view"}} â†’ {"user": {"id": 456, "action": "view"}}
  ```
...
- Swap out `id` arguments and nested object IDs in mutations and batch queries; verify perâ€‘object ownership checks.
...
- Presigned URLs (S3/GCS/Azure) are direct object references; require short expiries, method scoping, optional IP constraints, and backend reâ€‘validation on GET.
- Attempt to tweak the `Key=`/object path, query params, or credentials (e.g., `X-Amz-Security-Token`) to replay othersâ€™ files; test crossâ€‘tenant reuse.
...
- Verify explicit owner/tenant checks in Rego/Cedar; add unit tests for BOLA cases; denyâ€‘byâ€‘default at the first hop.

## Modern Detection & CI/CD Tooling

- Use Burp (Repeater/Intruder/Turbo Intruder) and authâ€‘diff extensions (e.g., Autorize/AuthMatrix) for parallel testing across roles.
