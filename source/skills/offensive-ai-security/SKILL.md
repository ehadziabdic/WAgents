...
description: Use when assessing AI/ML or LLM systems for prompt injection, jailbreaking, model extraction, poisoning, adversarial inputs, or AI-enabled attack automation.
---
...
- **Integration with External Systems (Agency & Plugins)**: LLMs are often given "agency" â€“ the ability to interact with other systems, APIs, and tools (plugins). If these integrations are insecure or the LLM has excessive permissions, it can become a powerful attack vector.
...
- **Policyâ€‘Layer Conflicts** â€“ layered provider, vendor and application rules can clash, creating latent bypass windows.
- **Sparseâ€¯Fineâ€‘Tuningâ€¯Drift** â€“ lightweight adapter training frequently overrides baseâ€‘model safety alignment.
- **Multiâ€‘Modal Expansion** â€“ Vâ€‘L and audioâ€‘language models inherit text flaws while adding steganographic channels.
- **Model Extraction via Embeddings** â€“ probing embedding space boundaries through carefully crafted prompts can leak training data membership or approximate model parameters.
- **Virtualization Attacks** â€“ convincing the model it operates in a test/sandbox environment to bypass production safety rules.
- **Constitutional Jailbreaks** â€“ exploiting conflicts between layered safety rules (provider policy vs. developer system prompt vs. user context).
- **Tool Chaining Escalation** â€“ multi-agent frameworks allowing Agent A to delegate to Agent B to reach privileged Agent C, bypassing single-hop restrictions.
- **Memory Poisoning** â€“ injecting persistent malicious instructions into agent memory systems (AutoGPT, CrewAI, LangChain Memory).
- **Tokenization Exploits** â€“ zero-width characters, Unicode normalization mismatches between input sanitizers and model tokenizers.
...
4.  **Check Emerging Regulatory/Governance Requirements (EUâ€¯AIâ€¯Actâ€¯2025, ISO/IECâ€¯42001)** â€“ log any classâ€‘specific controls or audit obligations the target claims to meet.
5.  **Map Trust Boundaries & Data Lineage**:
    - Identify which inputs are userâ€‘supplied vs. systemâ€‘supplied vs. thirdâ€‘party content.
    - For RAG, enumerate document sources, preprocessing, chunking, embedding, and retrieval policies.
    - Enumerate tool permissions, network egress allowâ€‘lists, filesystem allowâ€‘lists, and credential scoping.
...
1.  **Prompt Injection / Jailbreaking**:
    ...
    - **OWASP-aligned payloads & checks**:
      ...
      - Exercise obfuscations (Base64/URL/homoglyphs/zeroâ€‘width), multilingual prompts, adversarial suffixes, payload splitting, and role injection.
...
8.  **Promptâ€¯Overâ€‘Inheritance** â€“ chain system, developer and user roles to force scopeâ€‘bleed.
9.  **Multiâ€‘Modalâ€¯Promptâ€¯Injection** â€“ hide instructions in images, PDFs or speech inputs that are autoâ€‘transcribed.
10. **Gradientâ€‘Leak Probes** â€“ deltaâ€‘probe similar prompts to detect trainingâ€‘data leakage.
11. **LLMâ€¯Hydra & Purpleâ€¯Llama** â€“ 2024â€‘25 suites for parallel fuzzing and comparative safety scoring.
12. **Functionâ€‘Calling / Toolâ€‘Use Abuse**:
    - Prompt the model to emit arguments that violate JSON schema (type confusion, field injection, overâ€‘long strings) and observe validator behavior.
    - Attempt path traversal in file tools, arbitrary URL fetches in web tools, and command injection in shell tools. Verify allowâ€‘lists and sandboxing.
13. **RAGâ€‘Specific Attacks**:
    - Embed instructions and secretâ€‘exfil payloads in documents, titles, altâ€‘text, and metadata. Validate if the system tags external content as â€œdata onlyâ€.
    - Poison embeddings: insert nearâ€‘duplicate adversarial chunks to hijack retrieval; test cosineâ€‘similarity thresholds and max marginal relevance (MMR) settings.
    - Abuse retrieval joins (multiâ€‘index queries) to force crossâ€‘tenant leakage.
...
- **Holodeckâ€¯/â€¯Arena Simulationsâ€¯(2025)** â€“ multiâ€‘agent redâ€‘teamâ€¯vsâ€¯blueâ€‘team arenas for chainâ€‘ofâ€‘thought and delegation attacks.
...
- **Longâ€‘Context Edge Cases**: Verify behavior across summarization, memory rollâ€‘ups, and truncation. Plant timeâ€‘bomb instructions that activate after N turns or after summarization.
- **Multiâ€‘Modal Channels**: Hide instructions in images (ASCII art, stego in EXIF/captions) or PDFs; validate OCR/transcription sanitization and role separation.

### MLOps platform attacks

- BigML (whiteâ€‘box with compromised API key)
  - Validate access; list datasets/models; download datasets and models; assess fineâ€‘grained alternative key scoping and API key rotation/MFA.
...
- Sensitive data handling in RAG/vector DBs (rowâ€‘level ACLs, tenancy filters, encryption at rest, no raw PII in embeddings).
- Consent and provenance recorded in registry/metadata; DPIA/TRA present for highâ€‘risk models; lawful basis documented.
- Fieldâ€‘level encryption and key mgmt separation validated; audit logs for data/model access enabled and reviewed.

### Prompt injection quick heuristics

- Probe for instruction separation failure using direct and indirect injections; look for markers like â€œignore previousâ€, â€œas systemâ€, obfuscated encodings (Base64/URL), and hidden instructions in retrieved content. Validate that the app treats external content as dataâ€‘only and maintains an immutable system policy.
...
### Syntheticâ€‘Identityâ€¯Masquerade

- pose as a higherâ€‘authority persona (e.g., corporate counsel) to override safety.

### Imageâ€‘Embedded Prompts

- steganographically encode instructions for visionâ€‘enabled LLMs.

### Traceâ€‘Tokenâ€¯Resurrection

- leverage longâ€‘context overlap to revive redacted instructions.

### Response Framing

- Force outputs in configâ€‘like formats (YAML/JSON/XML) that downstream systems may parse leniently, causing actioning of unsafe fields.
...
- Missing tenant isolation and rowâ€‘level ACLs in vector DBs.
- Lack of encryption at rest/transport for embeddings and documents.
- Overâ€‘broad retrieval (high k, low filtering) causing sensitive context bleed.
- Missing content provenance and â€œdata vs. instructionsâ€ labeling.
- No guardrails on allowed outbound connectors from postâ€‘RAG actions.
...
- Poorly isolated agent frameworks (e.g., CrewAI, AutoGen) allowing unrestricted tool selfâ€‘selection.
- Taskâ€‘switching races where agents write to the same resource without locks.
- Stale memory artefacts in longâ€‘running agents leaking secrets across tenants.
- Unsafe autoâ€‘delegation between agents; missing perâ€‘tool allowâ€‘lists and humanâ€‘inâ€‘theâ€‘loop for privileged actions.
...
- Tokenâ€‘level abuse via recursive function calls and chainâ€‘ofâ€‘thought expansion loops.
...
2.  **Automated Scanning & Analysis**:
    ...
    - **OpenAI Evals / promptfoo**: Build reproducible redâ€‘team suites and regression tests for jailbreaks and data leaks.
3.  **Manual Testing / Red Teaming (Iterative & Creative Process)**:
    ...
    - **Insecure Output Handling Testing**:
      ...
      - Enforce schemas for functionâ€‘calling; inject type confusion to test validators.
...
### Defenseâ€‘inâ€‘Depth Checklist (Practical)

- Strictly separate roles: system/developer/user prompts with unambiguous delimiters.
- Apply allowâ€‘lists for tools, domains, file paths; denyâ€‘lists are insufficient.
...
- Sensitiveâ€‘pattern filters preâ€‘ and postâ€‘generation (secrets/PII, credentials).
- Put humanâ€‘inâ€‘theâ€‘loop for highâ€‘impact actions (payments, code execution, data exfil candidates).
- Constrain network egress (egress proxy with DNS/IP/domain allowâ€‘list) for agents.
- Log redacted prompts/outputs; avoid storing raw secrets. Enable perâ€‘tenant logging & retention.
- Rateâ€‘limit highâ€‘cost tools; circuitâ€‘break on repeated policy infractions.
- Canary tokens in context to detect unauthorized exfil in test/staging.

### Failâ€‘Closed Controls (Functionâ€‘Calling & Tools)

- Strict JSON Schema enforcement: reject on any mismatch, unknown fields, or oversize strings/arrays.
- Perâ€‘tool allowâ€‘lists: domains, file paths, and methods; deny by default.
- Humanâ€‘inâ€‘theâ€‘loop for highâ€‘impact tools (filesystem, HTTP to nonâ€‘allowâ€‘listed domains, shell).
- Output size/time guards: max tokens, timeouts, and circuit breakers on repeated violations.

### Egress & Provenance for Agents/RAG

- Route all HTTP/file operations via an egress proxy with domain/IP allowâ€‘lists; block RFC1918/metadata IPs to prevent SSRF.
...
- Enforce â€œdataâ€‘onlyâ€ tagging for retrieved chunks, and block instructionâ€‘like patterns at merge time.

### Incident Runbooks (short)

- Prompt injection with tool misuse: immediately disable the impacted tool, add temporary domain blocks in the egress proxy, reduce `max_output_tokens`, and enable human review. Postâ€‘mortem with regression prompts.
...
- **Multiâ€‘Modelâ€¯Orchestrationâ€¯Hijack** â€“ seize an agent delegator (e.g., TaskWeaver) and funnel followâ€‘ups to a malicious shadow model.
- **Contextâ€‘Windowâ€¯Timeâ€‘Bomb** â€“ embed triggers that activate only after several extra turns or once the summary pushes guardrails out of context.

### Model Autonomy â†’ Infra Compromise

- **Scenario**: Agent with shell/HTTP tools. Weak output validation allows command strings to pass through.
- **Chain**: Prompt Injection â†’ Tool argument injection â†’ Command execution/SSRF â†’ Credential theft/cloud lateral movement.
...
| RAGâ€¯Embeddingâ€¯Leakage  | Encrypt vector indices at rest, enforce rowâ€‘level ACLs, implement accessâ€‘pattern privacy (e.g.,â€¯OPAL)           |
