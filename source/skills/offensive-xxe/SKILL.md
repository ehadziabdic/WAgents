---
name: offensive-xxe
description: Use when testing XML parsers, APIs, SOAP services, or file uploads for classic, blind, error-based, XInclude, or upload-based XXE behavior.
---
...
In practice, full remote code execution rarely stems from XXE alone; it typically requires language-specific gadgetsâ€”such as PHP's `expect://` wrapper or Java deserialization sinksâ€”which XXE merely helps reach.
...
- **Burp Suite Pro 2025.2+ (â€œBurp AIâ€)**: automatically chains scanner-found XXE with out-of-band callbacks for quicker triage.
...
- **libxml2 â‰¥ 2.13**: `XML_PARSE_NO_XXE` disables all external entity resolution by default.
- **Python â‰¥ 3.13**: standard `xml.*` modules forbid external entities; enable only via `feature_external_ges`.
...
```go
// Go â€“ standard encoding/xml does not resolve external entities
