...
| Task | Approach |
|---|---|
| **Create** or **edit** with formulas/formatting | `openpyxl` â€” see gotchas below |
| **Bulk data** in or out | `pandas` (`read_excel`, `to_excel`) |
| **Quick look** at a sheet | `markitdown file.xlsx` â€” `## SheetName` per sheet; reads `.xlsm` too. No cell coordinates, so don't plan edits from it |
| **Read** a model (formulas *and* values) | two `load_workbook` passes â€” see gotchas |

> `openpyxl`, `pandas`, and `markitdown` are preinstalled â€” do not run `pip install` first; write the script and import directly. Only if an import fails (or the `markitdown` command is missing): `pip install` the missing package.
...
- **Document every assumption and hardcoded number** where the reader will see it â€” a cell comment, or an adjacent cell at a table's end. Cite a real source when one exists (`Source: Company 10-K, FY2024, Page 45, Revenue Note, [SEC EDGAR URL]`); when the number came from the user, say so plainly.
...
- **Editing an existing file: match its conventions exactly.** They override every guideline here. Find its designated input cells first â€” a distinct font color, fill, or shading marks them â€” write only there, and leave every existing formula untouched.

## Recalculate (mandatory whenever the file contains formulas)

openpyxl writes formulas as strings with **no cached values**. Until you recalculate, every
formula cell reads back as `None` to anything reading cached values â€” `pandas`,
...
withheld â€” trust `total_errors`, not the length of the list). Fix what it names and run it
again. **JSON with an `error` key instead of a `status` means nothing was recalculated**, and
only that case exits non-zero â€” `errors_found` exits 0, so never treat a clean exit as a clean
workbook.
...
Write 2â€“3 formulas first and check they pull the values you expect, before building out a grid.

**A workbook that links to another file loses those links** if you re-save it with openpyxl and
then recalculate. Such a formula reads `='[1]Returns Analysis'!$B$2` â€” the `[1]` is an index
...
â€” copy those cells' values out of the original before you save over them (`--force` overrides,
...
- **Prefer Excel-2007-era functions** â€” `SUMIFS`, `INDEX`, `MATCH`, `IFERROR`, `SUMPRODUCT` â€” which need no prefix.
...
- **`data_only=True` is destructive if you save.** That workbook has no formulas left, so saving replaces every one with a literal â€” permanently.
- **`data_only=True` on a file openpyxl just wrote returns `None` everywhere** â€” run `recalc.py` first. (A formula whose result is `""` also reads back as `None`.)
...
**Color:** blue text (`0,0,255`) for hardcoded inputs and scenario levers Â· black for formulas Â·
green (`0,128,0`) for links to another sheet Â· red (`255,0,0`) for links to another file Â·
yellow fill (`255,255,0`) for key assumptions and cells the user should fill in.

**Numbers:** currency `$#,##0`, with the unit named in the header (`Revenue ($mm)`) Â· zeros
render as `-`, including in percentages (`$#,##0;($#,##0);-`) Â· negatives in parentheses Â·
percentages `0.0%`, **stored as fractions** (`0.15` renders `15.0%`; storing `15` renders
`1500.0%`) Â· valuation multiples `0.0x` Â· years as text (`"2024"`, never `2,024`).

**Structure:** every assumption in its own labeled cell, referenced by the formulas that use it
(`=B5*(1+$B$6)`, never `=B5*1.05`) Â· formulas consistent across every projection period, since a
lone edited cell mid-row is the commonest silent error Â· guard denominators that can be zero.

## Dependencies

`openpyxl`, `pandas`, `markitdown` (pip, preinstalled â€” install only if an import fails or the command is missing) Â· LibreOffice (`soffice`, auto-configured for sandboxed environments via `scripts/office/soffice.py`)
