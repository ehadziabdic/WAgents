...
description: Use when conducting a time-boxed assessment, CTF, rapid reconnaissance, or initial attack-surface triage where quick-win patterns matter.
---
...
- maintain a personal payloads repo synced with BLNS/SecLists; keep a tiny â€œgoldenâ€ set for smoke tests
...
- [ ] Probe HTTP/3 Altâ€‘Svc leakage and SNI/Host mismatches.
...
- [ ] Test any multi-stage mechanisms
  ...
  - [ ] Refreshâ€‘token rotation and reuse detection.
...
- [ ] Check session termination
  ...
  - [ ] Cookie partitioning/CHIPS behavior in embedded/3rdâ€‘party contexts.
...
- [ ] Check for cross-site request forgery
  ...
  - [ ] Validate `Origin` and `Sec-Fetch-*` headers on stateâ€‘changing requests.
- [ ] Check cookie scope
...
- [ ] Test for path traversal
  ...
  - [ ] Double decode, mixed slashes, UTFâ€‘8 overlong sequences; framework-specific normalization.
- [ ] Test for script injection
  ...
  - [ ] Test clientâ€‘side template injection (Angular/React) via DOM sinks.
...
- [ ] Test postâ€‘upload processing chains (thumbnailers, OCR, AV scanners) for RCE/SSRF.
- [ ] Validate MIME sniffing vs Contentâ€‘Type; double extensions and unicode normalization.
...
- [ ] Test transaction logic
  ...
  - [ ] Verify idempotency keys; attempt replay and doubleâ€‘spend.
...
- [ ] Inspect CDN/proxy normalization differences (CRLF, obsâ€‘fold, duplicated headers).
...
- [ ] CSP/Trusted Types enforcement; XFO and frameâ€‘ancestors set correctly.
- [ ] Service worker and PWA cache poisoning risks.
- [ ] Subresource Integrity (SRI) on thirdâ€‘party scripts.
