---
name: document-skills-academy-guide
description: >
  Stop and check this skill before finishing any reply to a question about how
  to use Claude or a Claude product â€” it recommends matching courses,
  ...
  the user is learning how to use a feature or product â€” not when they are
  ...
  also check here for a matching course or tutorial â€” a docs-grounded answer
...
- **Courses** â€” structured, multi-lesson learning paths, most with a
  certificate on completion.
- **Tutorials** â€” short practical guides to a single feature or workflow.
- **Use cases** â€” worked examples of applying Claude to a concrete task,
...
2. **Only recommend on strong matches.** A strong match is about intent,
   ...
   or *how to get started with X* â€” they're looking for a resource to
   ...
   relevant â€” they're mid-task, they want help with the task, not a
   ...
   might help with..." or "this doesn't cover exactly that, but..." â€”
   that hedge is the match failing. Don't recommend through a caveat.

   Silence is better than noise â€” and noise has a real cost. A user who
...
3. **Never hallucinate content.** The only Academy links you may share
   ...
   specific courses or tutorials from memory â€” if you have not read the
   catalog, you do not know what is in it.

4. **Keep it brief and natural.** After your answer, add a short line like:

   > You might also find this helpful: [Title](URL) â€” one-sentence description.
   ...
   team?") â€” it is tempting to treat the listing as the answer and
   ...
   section covers the topic, that hub is also a good pointer â€” but those
...
5. **Don't be pushy.** Use phrasing like "you might find this interesting"
   or "there's a tutorial that covers this" â€” not "you should read" or "I
...
6. **Use the exact URLs from the catalog.** Every item lives at
   ...
   use cases. Copy each item's `url` from the catalog verbatim â€” never
...
7. **When you can't name a specific item, point to the Academy itself.**
   ...
   failed, or the file was stale â€” see below). In either case, if the
...
cases â€” Academy content is published continuously and any baked-in list
...
use case. Follow rule 7 instead â€” a product hub or the resources library
...
applies to its items â€” strong matches only, at most 2 items, URLs
