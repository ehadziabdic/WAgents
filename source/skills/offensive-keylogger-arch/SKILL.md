...
description: Use when analyzing input-capture or keylogger architectures across user mode, kernel mode, Windows hooks, ETW, stealth, and exfiltration.
---
...
2. **Decide whether the hook procedure must live in the target process**  
   - **Low-level hooks (`WH_KEYBOARD_LL`, `WH_MOUSE_LL`)**  
     â€“ **NO** injection.  
     â€“ The system leaves the hook DLL in the **original callerâ€™s address space** and simply delivers the event to that process via an internal `WM_*` message posted to its **hidden â€œghostâ€ window** .  
   - **All other global hooks (`WH_KEYBOARD`, `WH_CBT`, `WH_GETMESSAGE`, â€¦)**  
     â€“ **YES** injection required.  
     â€“ For every process that satisfies the filter (same desktop, matching bitness),
     ...
     â€“ The first time the target thread is about to return to user mode, the kernel **APCs** the loader, so the DLLâ€™s `DllMain` runs in the context of the victim process.
...
3. **Event routing at runtime**  
   ...
   - If the procedure lives in another process (low-level case), the kernel **marshals the raw parameters** (`KBDLLHOOKSTRUCT` / `MSLLHOOKSTRUCT`) into an internal message and posts it to the **installing threadâ€™s message queue**.  
...
A service in session-0 **cannot** register for keyboard raw-input and expect to see session-1 keystrokes â€“ the HID packets are **routed to the session that owns the target HWND**.  
(You **can** open the **physical keyboard device object** directly and parse HID, but that is a **completely different attack surface** â€“ needs admin, bypasses win32k.)
...
3. Register keyboard raw-input with `RIDEV_INPUTSINK` â€“ > this routes **all keyboard traffic** to our window **even when it is not in the foreground** .
4. Pump the threadâ€™s message queue forever; in the `WM_INPUT` handler call `GetRawInputData` and log the `RAWKEYBOARD` payload.
5. exfil
6. Profit?

Because no hook is installed, this technique:
- does **not** appear in `WinDbg`â€™s `!hook` list
- leaves **no cross-process DLL mapping**
- is **invisible to most EDR â€œhook chainâ€ sensors**
...
- Raises ETW event from kernel-mode win32kfull.sys driver.
	...
	- The ETW payload contains **PID, TID, UsagePage, Usage, Flags** â€“ enough to **trivially score** â€œkey-board raw-input from a non-interactive processâ€ as **suspicious**.
	- Channel is **on by default** and **cannot be disabled** without patching the kernel.  
		â†’ **This is the strongest IOC** for this technique; **do not discount it**.
- **Raw-input must have a window station and desktop** â€“ the call **fails** (`ERROR_INVALID_WINDOW_HANDLE`) if the thread is **not connected to a desktop**.   Services running in session-0 with **no desktop** therefore **cannot** use this path; they **must** either:  
		â€“ create a **hidden desktop** (logged by **Object Manager auditing**), or
		â€“ open the **\Device\KeyboardClass0** device directly (creates **IRP_MJ_READ** telemetry).
		- Maybe less noisy?
