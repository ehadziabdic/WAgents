...
description: Use when assessing endpoint protections or researching EDR/AV evasion involving hooks, direct syscalls, process manipulation, AMSI, ETW, or memory encryption.
---
...
- **Modern Context:**
  - Attackers also scan process memory for sensitive artifacts like authentication tokens. Coâ€‘pilot/IDE integrations, chat assistants, and browser extensions frequently cache Bearer/JWT tokens in memory.
  - Practical triage: search for `"Authorization: Bearer"`, `"eyJ"` (base64 JWT prefix), or providerâ€‘specific headers; dump minimal pages to avoid tripping antiâ€‘exfil rules.
...
- Telemetry minimization
  - Jitter longâ€‘lived channels; prefer namedâ€‘pipe/HTTP3 over noisy HTTP1; throttle upload intervals.
  - Use COM/runspace over PowerShell console to reduce scriptâ€‘block logs; avoid AMSIâ€‘flagged prologues.
- Cleanup
  - Remove services, tasks, drivers; restore SDDL; revert registry policy flips (WDAC/CI/Defender) and reâ€‘enable protections.
...
- Primary method: DeviceIoControl() â†’ IRP_MJ_DEVICE_CONTROL
...
- Deprecation/support scope (Microsoft):
  - Windows 11 â‰¤ 23H2: VBS enclaves are deprecated; existing enclaves signed with the legacy EKU (OID `1.3.6.1.4.1.311.76.57.1.15`) continue to run until re-signed. New enclave signing requires updated EKUs and is not supported on these versions.
  - Windows 11 24H2+ and Windows Server 2025: VBS enclaves are supported with new EKUs.
- Security fix: CVE-2024-49076 (VBS Enclave EoP) â€” ensure December 2024+ updates are applied.
...
- or obtain `SSN`(System Service Number) dynamically and call them(can be done via [SysWhispers2](https://github.com/jthuraisamy/SysWhispers2))
  ...
  - Major EDR vendorsnow flag **nonâ€‘ntdll syscall sites**; consider **returnâ€‘address replication gadgets** to reâ€‘insert a plausible `ntdll` frame before the transition.
...
#### Kernelâ€‘Mode EDR Killers (BYOVD)

- **Bringâ€‘Yourâ€‘Ownâ€‘Vulnerableâ€‘Driver (BYOVD)** attacks load legitimately signed but exploitable driversâ€”examples include **rtcore64.sys**, **iqvw64e.sys**, and **terminator.sys**â€”to execute privileged code inside the kernel.
- Typical payload actions
  - Patch or unregister kernelâ€‘mode notify callbacks (`PsSetCreateProcessNotifyRoutine`, `ObRegisterCallbacks`) to blind userâ€‘mode EDR components.
  - Overwrite or unload _WdFilter.sys_ and other sensor drivers, fully disabling Defender or thirdâ€‘party agents.
- Public toolchains such as **Terminator**, **kdmapper**, and **EDRSensorDisabler** automate these steps.
- Case study: Lenovo `LnvMSRIO.sys` (CVEâ€‘2025â€‘8061). Exposes physical memory/MSR readâ€‘write primitives to lowâ€‘privileged users; can overwrite MSR_LSTAR and pivot to Ringâ€‘0 payload, then patch/unregister callbacks to blind EDR.

> [!TIP]
> The vulnerableâ€‘driver blocklist (`DriverSiPolicy.p7b`) is **enabled by default** on Windowsâ€¯11â€¯22H2+ and refreshed every Patch Tuesday. Keep HVCI/KDP enabled so attacker drivers cannot patch protected code pages, and enable Hardwareâ€‘Enforced Stack Protection (CET/Shadowâ€¯Stack) on Windowsâ€¯11â€¯24H2 to break callâ€‘stack spoofing.

> [!NOTE]
> Because the blocklist is on by default and updated frequently, a BYOVD chain now often needs **two** vulnerable drivers: one to disable Secureâ€¯Boot or flip `CiOptions`, and a second to perform the EDRâ€‘killer actions before the next blocklist refresh.
...
> [!TIP]
> Symlink/junction UAC races are buildâ€‘dependent and brittle. Validate on the specific target build; many have partial or complete mitigations.
...
> [!CAUTION]
> Defender engineâ€¯1.417â€¯ reâ€‘hooks common byte patches within ~50 ms. Prefer **callback filtering** (e.g., wrapping `EtwEventWriteFull`) or the patchâ€‘less SharpBlock fork.
...
- Defender now ships dynamic signature **VirTool:Win64/HBPAmsibyp.A** that fires on known NOPâ€‘sledâ€¯prologues. A stealthier option is to swap to a COMâ€‘visible PowerShell runspace instead of patching `AmsiScanBuffer`.
...
- VEH-based bypass: Register vectored exception handlers to set hardware breakpoints on `amsi.dll!AmsiScanBuffer` and re-route execution to a benign stub without byte-patching (a.k.a. VEHÂ²). This avoids obvious prologue modifications.
- COM/vtable approach: Overwrite IAmsiStream vtable entries or CLSID pointers used during `AmsiInitialize` so the provider cannot be created and AMSI calls short-circuit â€” again without altering `AmsiScanBuffer` bytes.
- Defender considerations: 2024â€“2025 engines re-hook common byte patches rapidly; patchless flows reduce simple signature hits but still leave behavioral traces.
- Blue detections:
  - Monitor `AddVectoredExceptionHandler` usage paired with debug register changes (`CONTEXT.Dr0â€“Dr7`) shortly before AMSI invocations.
...
- 24H2+ Security Baseline
  - Hardwareâ€‘Enforced Stack Protection (CET/Shadow Stack) breaks naÃ¯ve callâ€‘stack spoofing; evasions must maintain valid shadow stacks.
  - LSASS protection (RunAsPPL) is on by default on recent fresh installs; expect minidump blockers. Favor liveâ€‘offâ€‘land telemetry for detection/response.
...
- Hunt eBPF program/map churn and sensor detaches; sidecars outside nodeâ€‘agent PID filters; attempts to access runtime sockets.
...
> [!CAUTION]
> Some EDR/XDR products flag sudden `/32` selfâ€‘IP assignments (host routes) created with `Newâ€‘NetIPAddress` as potential IP spoofing. Expect alerts on repeated adds/removes or tight timing around agent network failures. Prefer `/31` pairs or upstream firewall/DNAT approaches where possible.
...
#### Agent Tampering & Tamperâ€‘Protection Bypasses

- Microsoftâ€¯Defender's Tamper Protection (and similar vendor selfâ€‘protection) can be disabled by
  - Launching a TrustedInstallerâ€‘child process and modifying service registry keys (e.g., `sc config Sense start= disabled`).
  - Changing ACLs on `WdFilter` or `SenseIR` registry values, then rebooting so the filter driver never loads.
- Many commercial EDRs expose selfâ€‘protection flags via WMI namespaces; clearing the flag allows the agent service to be stopped or deleted.
...
#### Cloudâ€‘Side XDR Telemetry Throttling & Poisoning
...
- Attackers therefore:
  ...
  - Inject thousands of benign events (mass process creations) to drown anomalyâ€‘detection models (â€œtelemetry poisoningâ€).
- Blue teams should monitor for sudden policy flips or extreme eventâ€‘volume spikes with low entropy.
...
- Conditionalâ€‘Access policies hinge on properties like `trustType`, `deviceComplianceState`, and risk level.
- Attackers possessing the device certificate can invoke `DsregSetDeviceProperty()` to flip these flags, then run `dsregcmd /refreshprereqs` to push altered attributes to Entraâ€¯ID.
- **Impact:** stolen or cached tokens appear compliant, bypassing MFA or deviceâ€‘based restrictions until the next compliance refresh.
...
- Blue-team quick checks:
  - Verify registry: `HKLM\SYSTEM\CurrentControlSet\Control\Lsa` â†’ `RunAsPPL`/`RunAsPPLBoot` (policy may also store a UEFI variable for lock).
  - Confirm status with eventing and `Get-ProcessMitigation -System` and hardware readiness (`Get-CimInstance Win32_DeviceGuard`).

### Firmware & Bootâ€‘level Threats

- **UEFI Bootkits** (e.g., BlackLotus, CosmicStrandâ€‘2024) execute during the DXE phase, disable BitLocker and HVCI before Windows loads, and can patch kernel code pages before EDR starts.
- **Secureâ€¯Boot bypass CVEâ€‘2024â€‘7344** abuses an unâ€‘revoked testâ€‘signed shim to load unsigned payloads even with Secure Boot enabled.
- **Mitigations**
  - Keep DBX revocation list current (Augustâ€¯2024 or later).
...
- Tools like **Falcoâ€¯0.38+** , **Ciliumâ€¯Tetragon** (realâ€‘time enforcement with `kubectl tetragon observe`), and **AWSâ€¯GuardDutyâ€¯eBPF** hook `sys_enter` tracepoints and kprobes to emulate EDR telemetry on Kubernetes nodes.
- **Evasion**
  - Detach or overwrite the eBPF program with `bpftool prog detach`.
  - Remount `debugfs` elsewhere to hide BPF maps and programs.
  - Hide processes in sidecar containers that run outside the nodeâ€‘agent's PID namespace filter.

#### PIDâ€‘Namespace & Cgroup Tricks

- **CVEâ€‘2025â€‘26324** leaks host PIDs, allowing a container process to open `/proc/<hostpid>/mem` and bypass namespaceâ€‘based sensors.
- Sidecars running in the host network namespace can exfiltrate data without visibility from nodeâ€‘level agents.

### Nonâ€‘Windows Endpoints (macOS, iOS, Android)

#### macOS

- Starting with macOSâ€¯Sonomaâ€¯(14), every Gatekeeper verdict is logged via the unified log channel `sender=syspolicyd`. Blue teams monitor these streams; red teams should clear the quarantine flag **before** first execution or ship a notarised loader.
- Direct edits to **TCC.db** grant screenâ€‘recording, camera, or microphone entitlements without user prompts.
- Combine with notarised, adâ€‘hoc signed apps to bypass Gatekeeper on macOS 15.

#### Mobile

- **Androidâ€¯13/14**: SIMâ€‘swap plus deferred `appops` removes MDM profiles without factory reset.
...
- Windows now ships a quarterly updated vulnerableâ€‘driver blocklist; violations surface under _Device security â†’ Core isolation_.
- **Kernel Data Protection (KDP)** marks EDR code pages readâ€‘onlyâ€”even for kernel drivers.
- **Attacker response:** use driverâ€‘name collision (rename malicious driver to `iqvw64e.sys`) or flip _CiOptions_ from a BYOVD payload before loading unsigned code.
...
- Hardwareâ€‘Enforced Stack Protection (CET/Shadow Stack) is increasingly enabled (Windows 11 24H2+). Callâ€‘stack spoofing must account for CET or will fault.
